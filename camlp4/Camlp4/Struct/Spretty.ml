...
module Make (Loc : Sig.Loc.S) = struct
  module Loc = Loc;
  ...
  value print_comment _tab s _nl_bef _tab_bef _empty_stmt =
    print_string_fun.val s;
    (* if s = "" then ()
    else do {
  ...
  value rec conv =
    ...
    | LocInfo loc x ->
        List.fold_right (fun comm accu -> LI comm accu) (getcomm.val loc) (conv x) ]
  and conv_stream =
    parser
