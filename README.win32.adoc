...
* The Microsoft Visual C/C++ compiler does not implement "computed gotos" (also
  known as "labels as values"), and therefore generates inefficient code for the
  bytecode interpreter in `runtime/interp.c`.  Consequently, the performance of
  bytecode programs is about 2/3 of that obtained under Unix/GCC, Cygwin or
  MinGW-w64 on similar hardware. The clang-cl compiler supports the "computed
  gotos" extension.
