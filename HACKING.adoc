...
opam switch create . --empty # Alternatively use a global name instead of '.'
./configure -C --prefix=$(opam var prefix) # put extra configuration args here
make -j
make INSTALL_MODE=opam OPAM_PACKAGE_NAME=ocaml-variants install
opam pin add --kind=path ocaml-variants .
...
run `opam install .` from the compiler source root, but this will recompile the
compiler from scratch and also trigger a recompilation of all the packages in
the switch.

Using `opam install . --assume-built` will simply remove the
package for the compiler, then run the installation instructions
(`make install INSTALL_MODE=opam OPAM_PACKAGE_NAME=ocaml-variants`) in the
working directory, tracking the installed files correctly. However, there's no
way to prevent this command from triggering a full recompilation of your
switch.

You can also run the `make install INSTALL_MODE=opam
OPAM_PACKAGE_NAME=ocaml-variants` command manually, which will not trigger a
recompilation. There are no checks that it's safe to do so, so if your patch
ends up changing even slightly one of the core libraries' files, you will
likely get inconsistent assumptions errors later.
