...
INRIA's CI runs the script tools/ci-build. In particular, when running
the CI on a publicly available branch as explained in the previous
section, you can edit this script to change what the CI will test.

For instance, parallel builds are only tested for the main builds. In
order to test parallel builds with a custom branch, add this at the
beginning of tools/ci-build:
