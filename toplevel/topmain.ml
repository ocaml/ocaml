  ...
  let native_preload =
    Toploop.is_native &&
      (Filename.check_suffix name ".cmxs"
       ...
       || Filename.check_suffix name ".cmxa") in
  if byte_preload || native_preload
...
module ByteOptions = Main_args.Make_bytetop_options (struct
  let set r () = r := true
  let clear r () = r := false
...
module OptOptions = Main_args.Make_opttop_options (struct
  let set r () = r := true
  let clear r () = r := false
...
let main () =
  native_code := Toploop.is_native;
  let ppf = Format.err_formatter in
  let list = if Toploop.is_native then OptOptions.list else ByteOptions.list in
  Compenv.readenv ppf Before_args;
  Arg.parse list file_argument usage;
  Compenv.readenv ppf Before_link;
