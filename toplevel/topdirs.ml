...
let _ = add_directive "directory" (Directive_string dir_directory)
    "<string> Add the given directory to search path for source and compiled files."
...
let _ = add_directive "remove_directory" (Directive_string dir_remove_directory)
    "<string> Remove the given directory from the search path."
...
let _ = add_directive "cd" (Directive_string dir_cd)
    "<string> Change the current working directory."
...
let _ = add_directive "load" (Directive_string (dir_load std_out))
    "<string> Load in memory a bytecode object, produced by ocamlc."
...
let _ = add_directive "load_rec"
    ...
    "<string> As #load, but loads dependencies recursively."
...
let _ = add_directive "use" (Directive_string (dir_use std_out))
    "<string> Read, compile and execute source phrases from the given file."

let _ = add_directive "mod_use" (Directive_string (dir_mod_use std_out))
    "<string> Usage is identical to #use but #mod_use wraps the contents in a module."
...
let _ = add_directive "install_printer"
    ...
    "<ident> Registers a printer for values of a certain type."

let _ = add_directive "remove_printer"
    (Directive_ident (dir_remove_printer std_out))
    "<ident> Remove the named function from the table of toplevel printers."
...
let reg_show_prim name to_sig descr =
...
let _ = add_directive "print_depth"
    ...
    "<int> Limit the printing of values to a maximal depth of n."

let _ = add_directive "print_length"
    (Directive_int(fun n -> max_printer_steps := n))
    "<int> Limit the number of value nodes printed to at most n."

(* Set various compiler flags *)

let _ = add_directive "labels"
    (Directive_bool(fun b -> Clflags.classic := not b))
    "<bool> Choose whether to ignore labels in function types."

let _ = add_directive "principal"
    (Directive_bool(fun b -> Clflags.principal := b))
    "<bool> Make sure that all types are derived in a principal way."
...
let _ = add_directive "ppx"
    ...
    "<string> After parsing, pipe the abstract \
     syntax tree through the preprocessor command."

let _ = add_directive "warnings"
    (Directive_string (parse_warnings std_out false))
    "<string> Enable or disable warnings according to the argument."

let _ = add_directive "warn_error"
    (Directive_string (parse_warnings std_out true))
    "<string> Treat as errors the warnings enabled by the argument."
...
let _ = add_directive "help"
    ...
    "Prints a list of all available directives, with \
