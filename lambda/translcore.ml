...
let transl_extension_constructor ~scopes env path ext =
  ...
  match ext.ext_kind with
    Text_decl _ ->
      Lprim (Pmakeblock (Obj.object_tag, Immutable, None, Block_desc.empty),
...
and transl_exp0 ~in_new_scope ~scopes e =
  ...
  | Texp_construct(_, cstr, args) ->
      ...
      | Cstr_extension(path, is_const) ->
          ...
          if is_const then lam
            ...
            Lprim(Pmakeblock(0, Immutable, Some (Pgenval :: shape), Block_desc.empty),
  ...
  | Texp_atomic_loc (arg, _, lbl) ->
      ...
      Lprim (Pmakeblock (0, Immutable, shape, Block_desc.empty), [arg; lbl], loc)
