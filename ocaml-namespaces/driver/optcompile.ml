...
let interface ppf sourcefile outputprefix =
  ...
  Nsutils.set_unit_name modulename;
  ...
  try
    ...
    let sg = Typemod.transl_interface sourcefile (initial_env()) ast in
    if !Clflags.print_types then
      fprintf std_formatter "%a@." Printtyp.signature
                                   (Typemod.simplify_signature sg);
    Warnings.check_fatal ();
    if not !Clflags.print_types then
      Env.save_signature sg (Nsutils.modulename()) (outputprefix ^ ".cmi");
...
let implementation ppf sourcefile outputprefix =
  ...
  try
    let parsetree = Pparse.file ppf inputfile Parse.implementation ast_impl_magic_number in
    if !Clflags.print_types then ignore(
      parsetree
      ++ print_if ppf Clflags.dump_parsetree Printast.implementation
      ++ Unused_var.warn ppf
      ++ Typemod.type_implementation sourcefile outputprefix (Nsutils.modulename()) env)
    else begin
      parsetree
      ++ print_if ppf Clflags.dump_parsetree Printast.implementation
      ++ Unused_var.warn ppf
      ++ Typemod.type_implementation sourcefile outputprefix (Nsutils.modulename()) env
      ++ Translmod.transl_store_implementation (Nsutils.modulename())
      +++ print_if ppf Clflags.dump_rawlambda Printlambda.lambda
      +++ Simplif.simplify_lambda
      +++ print_if ppf Clflags.dump_lambda Printlambda.lambda
      ++ Asmgen.compile_implementation outputprefix ppf  (Nsutils.modulename());
