...
let mk_namespace f =
  "-namespace", Arg.String f, "<namespace> : build in namespace <namespace> (use - to infer from relative path)"

let mk_open_namespace f =
  "-open-namespace", Arg.String f, "<namespace> : open namespace <namespace>"
...
module type Bytecomp_options = sig
  ...
  val _namespace : string -> unit
  val _open_namespace : string -> unit
...
struct
  let list = [
    ...
    mk_where F._where;
    mk_namespace F._namespace;
    mk_open_namespace F._open_namespace;
    mk_debug F._debug;
