...
let implementation ppf sourcefile outputprefix =
  ...
  if !Clflags.print_types then begin
    try ignore(
      let parsetree = Pparse.file ppf inputfile Parse.implementation ast_impl_magic_number
      in
      parsetree
    ...
    try
      ...
      ++ Unused_var.warn ppf
      ++ Typemod.type_implementation sourcefile outputprefix (Nsutils.modulename()) env
      ++ Translmod.transl_implementation (Nsutils.modulename ())
      ++ print_if ppf Clflags.dump_rawlambda Printlambda.lambda
      ++ Simplif.simplify_lambda
      ++ print_if ppf Clflags.dump_lambda Printlambda.lambda
      ++ Bytegen.compile_implementation (Nsutils.modulename())
      ++ print_if ppf Clflags.dump_instr Printinstr.instrlist
      ++ Emitcode.to_file oc (Nsutils.modulename());
      Warnings.check_fatal ();
      close_out oc;
      Pparse.remove_preprocessed inputfile;
