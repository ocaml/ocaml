...
implementation:
      IN mod_longident structure EOF       {
	if !Clflags.debug_ns then
	  Printf.fprintf stderr "Parser set ns_current_in_file\n%!";
        Clflags.ns_current_in_file := Some (Longident.name $2);
	$3 }
    | structure EOF                        {
      if !Clflags.debug_ns then
	Printf.fprintf stderr "Parser set ns_current_in_file\n%!";
  ...
  ;
interface:
      IN mod_longident signature EOF       {
	if !Clflags.debug_ns then
	  Printf.fprintf stderr "Parser set ns_current_in_file\n%!";
	Clflags.ns_current_in_file := Some (Longident.name $2);
	$3 }
