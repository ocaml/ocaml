...
and module_components_repr =
  ...
  | Functor_comps of functor_components
      | Namespace_comps of namespace_components

    and namespace_components = {
      mutable nscomp_modules : (string, (Path.t * module_type Lazy.t)) Tbl.t;
      mutable nscomp_components : (string, (Path.t * module_components)) Tbl.t;
    }
...
(* Shall-we accept to use namespaces as -I ? I think No!
&&
      (match !Clflags.ns_current_in_file with
	  Some _ ->
	    (* ns-namespace: allow b/x.cmi to contains X instead of B.X
	    (i.e. use of namespaces to replace -I) *)
	    (try
	      let s = modname in
	      let pos = String.rindex s '.' in
	      let name = String.sub s (pos+1) (String.length s - pos - 1) in
	      name <> ps.ps_name
	  with _ -> true)
	| None -> (* otherwise, we only need the last name to match *)
	  try
	    let s = ps.ps_name in
	    let pos = String.rindex s '.' in
	    let name = String.sub s (pos+1) (String.length s - pos - 1) in
...
*)
    then
          raise(Error(Illegal_renaming(ps.ps_name, modname, filename)));
    ...
    List.iter
      ...
      ps.ps_flags;
	if !Clflags.debug_env then
	  Printf.fprintf stderr "Loaded persistent %s\n%!" modname;
...
let rec find_module_descr path env =
  match path with
    Pident id ->
      ...
      with Not_found ->
	    if !Clflags.debug_env then
	      Printf.fprintf stderr "Find identifier %s\n%!" (Ident.name id);
  ...
  | Pdot(p, s, pos) ->
      begin match Lazy.force(find_module_descr p env) with
        Structure_comps c ->
          ...
          descr
            | Namespace_comps c ->
              let (path, descr) = Tbl.find s c.nscomp_components in
              descr
...
let find proj1 proj2 path env =
  ...
  | Pdot(p, s, pos) ->
      begin match Lazy.force(find_module_descr p env) with
	    ...
	    | Namespace_comps _ -> raise Not_found
      | Functor_comps f ->
          raise Not_found
      end
  | Papply(p1, p2) ->
...
let find_module path env =
  ...
  | Pdot(p, s, pos) ->
      begin match Lazy.force (find_module_descr p env) with
	    ...
	    | Namespace_comps c ->
		let (path, data) = Tbl.find s c.nscomp_modules in Lazy.force data
      | Functor_comps f ->
          raise Not_found
      end
  | Papply(p1, p2) ->
      ...
      begin match Lazy.force descr with
        Structure_comps c ->
          ...
          (Pdot(p, s, pos), Lazy.force data)
            | Namespace_comps c ->
              let (path, data) = Tbl.find s c.nscomp_modules in
              (path, Lazy.force data)
      | Functor_comps f ->
          raise Not_found
      end
  | Lapply(l1, l2) ->
  ...
  | Ldot(l, s) ->
      ...
      begin match Lazy.force desc with
        Structure_comps c ->
          ...
          (Pdot(p, s, pos), data)
            | Namespace_comps c -> raise Not_found
      | Functor_comps f ->
          raise Not_found
      end
  | Lapply(l1, l2) ->
...
let rec prefix_idents root pos sub = function
  ...
  | Tsig_module(id, mty, _) :: rem ->
	  if !Clflags.debug_env then
	    Printf.fprintf stderr "prefix_idents %s (pers: %b)\n%!" (Ident.name id) (Ident.persistent id);
      let p = Pdot(root, Ident.name id, pos) in
      let (pl, final_sub) =
...
(* Compute structure descriptions *)
	...
	let name = Ident.name id in
	  Ident.change_name id (ns_short_name name)
      with Not_found -> id
	...
	| Tmty_namespace (name, sg) ->
	  let c =
	    {
	      nscomp_modules = Tbl.empty;
	      nscomp_components = Tbl.empty;
	    } in
	  ...
	  List.iter (fun item (* path *) ->
	    match item with
(*
	      | Tsig_module(id, (Tmty_namespace _  as mty), _) ->
		let path = Pident id in
		Printf.fprintf stderr "introducing sub-namespace element id=%s with path=%s\n%!" (Ident.name id) (Path.name path);
		let mty' = lazy ( Subst.modtype sub mty) in
		c.nscomp_modules <-
		  Tbl.add (Ident.name id) (path, mty') c.nscomp_modules;
		let comps = components_of_module !env sub path mty in
		c.nscomp_components <-
		  Tbl.add (Ident.name id) (path, comps) c.nscomp_components;
		env := store_module id path mty !env
*)
	      | Tsig_module(id, mty, _) ->
		if !Clflags.debug_ns then
		  Printf.fprintf stderr "introducing sub-module element id=%s with path=%s\n%!" (Ident.name id) (Path.name path);
		assert (Ident.persistent id);
		let path = Pident id in
		let mty' = lazy ( Subst.modtype sub mty) in

		let id = ns_short_id id in
		let modname = Ident.name id in
		c.nscomp_modules <-
		  Tbl.add modname (path, mty') c.nscomp_modules;
		let comps = components_of_module !env sub path mty in
		c.nscomp_components <-
		  Tbl.add modname (path, comps) c.nscomp_components;
		env := store_module id path mty !env
	      | Tsig_value _
	      | Tsig_type _
	      | Tsig_exception _
...
let open_signature root sg env =
      if !Clflags.debug_env then
	Printf.fprintf stderr "open_signature %s\n%!" (Path.name root);
  ...
  let newenv =
    List.fold_left2
      (fun env item p ->
        ...
        | Tsig_module(id, mty, _) ->
(*		let id = ns_short_id id in
		Printf.fprintf stderr "open: store %s in env\n%!" (Ident.name id); *)
  ...
  { newenv with summary = Env_open(env.summary, root) }
    ...
    let open_namespace root sg env =
      if !Clflags.debug_ns then
	Printf.fprintf stderr "open_namespace %s\n%!" (Path.name root);
      (* First build the paths and substitution *)
(*      let (pl, sub) = prefix_idents root 0 Subst.identity sg in *)
      (* Then enter the components in the environment after substitution *)
      let newenv =
		...
		if not (Ident.persistent id) then begin
		  Printf.fprintf stderr "%s is not persistent !\n%!" (Ident.name id);
		  assert false;
		end;
		let p = Pident id in
		let id = ns_short_id id in
		if !Clflags.debug_ns then
		  Printf.fprintf stderr "open: store %s in env\n%!" (Ident.name id);
                store_module id p ( (*Subst.modtype sub*) mty) env
            | Tsig_value _
            | Tsig_type _
            | Tsig_exception _
            | Tsig_modtype _
...
(* Read a signature from a file *)
    ...
    let find_pers_sig name =
      let ps = find_pers_struct name in
	(ps.ps_filename, ps.ps_sig)
...
let report_error ppf = function
  ...
  | Corrupted_interface filename -> fprintf ppf
      ...
      | Illegal_renaming(modname, expected_modname, filename) -> fprintf ppf
          "Wrong file naming: %s@ contains the compiled interface for@ %s instead of %s"
          filename modname expected_modname
