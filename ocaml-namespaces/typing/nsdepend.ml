...
(* $Id: depend.ml 10738 2010-10-21 23:59:33Z garrigue $ *)
...
(* Problems with namespaces:
  * module S = struct ... end;; open X;; open S;; we should create a dependency
    here to s.cmi, since s.cmi can be in namespace x/. Consequently,
    we clear bv after open and let open. <<namespace 1>>
  * X.Y : if X is free, then Y should be too. <<namespace 2>>

Add a flag -namespace xxx, so that we change the behavior of ocamlpro
only if that flag is on.
...
let rec addmodule bv lid =
  match lid with
    ...
    | Ldot(l, s) ->
      let old_free = !free_structure_names in (* <<namespace 2>> *)
      addmodule bv l;
      if old_free != !free_structure_names then
	free_structure_names := StringSet.add s !free_structure_names
...
let rec add_expr bv exp =
  ...
  | Pexp_open (m, _, e) ->   (* <<namespace 1>> *)
    addmodule bv m; add_expr StringSet.empty e
...
let add_structure bv m =
  ignore (add_structure bv m)
