...
let extract_sig_open env loc mty =
  match Mtype.scrape env mty with
      Tmty_signature sg -> sg
    | Tmty_namespace (_,sg) -> Lazy.force sg
...
*)
  match alias with
	...
	if !Clflags.debug_ns then
	  Printf.fprintf Pervasives.stderr "type_open %s\n%!" (Longident.name lid);
...
let rec type_module_no_check sttn funct_body anchor env smod =
...
and type_module sttn funct_body anchor env smod =
  let mdesc = type_module_no_check sttn funct_body anchor env smod in
  begin
    match mdesc.mod_type with
	Tmty_namespace (name, _) ->
	  raise(Error(mdesc.mod_loc, Namespace_as_structure name))
      | _ -> ()
  end;
  mdesc
...
let transl_interface sourcefile initial_env ast =
  let newenv = Nsutils.add_signature_namespaces initial_env ast sourcefile in
  transl_signature newenv ast
...
let type_implementation sourcefile outputprefix modulename initial_env ast =
  ...
  let has_interface_file =  Sys.file_exists sourceintf in
  let ns_initial_env = Nsutils.add_structure_namespaces (if has_interface_file then Some modulename else None)
    initial_env ast sourcefile in
  let (str, sg, finalenv) = type_structure ns_initial_env ast Location.none in
...
let report_error ppf = function
  ...
  | Incomplete_packed_module ty ->
      fprintf ppf
        ...
        type_expr ty
  | Namespace_as_structure name ->
    fprintf ppf
      "Namespace %s is illegaly used as a structure" name
