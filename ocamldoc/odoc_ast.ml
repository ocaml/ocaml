              ...
              Simple_name { sn_name = "*opt*" } ->
                (
                 (
                  ...
                  | _ ->
                      print_DEBUG3 "Pas le bon filtre pour le param�re optionnel avec valeur par d�aut.";
                      (parameter, func_body)
                 )
                )
            | _ ->
...
*)
        ...
        (Parsetree.Pcl_constr (lid, _), tt_class_exp_desc ) -> 
          let name = 
            ...
            | _ ->
                ...
                (* A VOIR : dommage qu'on n'ait pas un Tclass_ident :-( m�e quand on a class tutu = toto *)
                Name.from_longident lid
          in
          (* On n'a pas ici les param�res de type sous forme de Types.type_expr,
   ...
   and analyse_structure_item env current_module_name loc pos_limit comment_opt parsetree_item_desc typedtree 
	     ...
	     in
             ...
             let new_env2 = 
               match new_module.m_type with
                 (* A VOIR : cela peut-il �re Tmty_ident ? dans ce cas, on aurait pas la signature *)
      ...
      | Parsetree.Pstr_modtype (name, modtype) ->
          ...
          let new_env2 =
            match tt_module_type with 
              (* A VOIR : cela peut-il �re Tmty_ident ? dans ce cas, on n'aurait pas la signature *)
              Types.Tmty_signature s -> 
      ...
      | Parsetree.Pstr_include module_expr ->
          ...
          in
          (0, env, [ Element_included_module im ]) (* A VOIR : �endre l'environnement ? avec quoi ? *)
