...
CAMLprim value caml_dynlink_open_lib(value mode, value filename)
{
  ...
  result = caml_alloc_small_loc(1, Abstract_tag, PROF_DYNLINK);
...
CAMLprim value caml_dynlink_get_current_libs(value unit)
{
  ...
  res = caml_alloc_tuple_loc(shared_libs.size, PROF_TUPLE);
  for (i = 0; i < shared_libs.size; i++) {
    value v = caml_alloc_small_loc(1, Abstract_tag, PROF_DYNLINK);
