...
#define Setup_for_gc \
  { sp -= 2; sp[0] = accu; sp[1] = env; caml_domain_state->extern_sp = sp; }
#define Restore_after_gc \
  { sp = caml_domain_state->extern_sp; accu = sp[0]; env = sp[1]; sp += 2; }
#define Enter_gc \
  { Setup_for_gc; caml_handle_gc_interrupt(); Restore_after_gc; }
#define Setup_for_c_call \
  { saved_pc = pc; *--sp = env; caml_domain_state->extern_sp = sp; }
#define Restore_after_c_call \
  { sp = caml_domain_state->extern_sp; env = *sp++; saved_pc = NULL; }
...
#define Restore_after_event \
  { sp = caml_domain_state->extern_sp; accu = sp[0]; \
...
value caml_interprete(code_t prog, asize_t prog_size)
{
  ...
  struct caml_exception_context exception_ctx = { &raise_buf, caml_domain_state->local_roots};
...
#include "caml/jumptbl.h"
  ...
  if (sigsetjmp(raise_buf.buf, 0)) {
    ...
    accu = caml_domain_state->exn_bucket;
...
/* Exceptions */
    ...
    Instruct(PUSHTRAP):
      ...
      Trap_link(sp) = Val_long(caml_domain_state->trap_sp_off);
      sp[2] = env;
      sp[3] = Val_long(extra_args);
      caml_domain_state->trap_sp_off = sp - caml_domain_state->stack_high;
      pc++;
      Next;
    ...
    Instruct(POPTRAP):
      ...
      caml_domain_state->trap_sp_off = Long_val(Trap_link(sp));
      sp += 4;
      Next;
    ...
    raise_notrace:
      if (caml_domain_state->trap_sp_off > 0) {
        if (Stack_parent(caml_domain_state->current_stack) == Val_long(0)) {
          caml_domain_state->external_raise = initial_external_raise;
          caml_domain_state->trap_sp_off = initial_trap_sp_off;
          caml_domain_state->extern_sp = caml_domain_state->stack_high - initial_stack_words ;
          caml_callback_depth--;
        ...
        sp = caml_domain_state->stack_high + caml_domain_state->trap_sp_off;
...
/* Context switching */
    ...
    Instruct(RESUME):
      ...
      resume_arg = sp[1];
      sp -= 3;
      sp[0] = Val_long(caml_domain_state->trap_sp_off);
      sp[1] = Val_long(0);
      sp[2] = Val_pc(pc);
      sp[3] = env;
      sp[4] = Val_long(extra_args);
