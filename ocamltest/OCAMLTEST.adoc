...
insights will be deepened in <<concepts>> where ocamltest is
...
own tests. Refer to
xref:../INSTALL.adoc[] (and also to xref:../README.win32.adoc[] if you are on Windows) to
...
and compiled them as described in xref:../INSTALL.adoc[] or xref:../README.win32.adoc[],
...
. First, we add a special comment at the very beginning of our `hello.ml`
...
. We can now ask ocamltest to run our test program with the
  following command:
+
[source,bash]
----
...
compiler. So, as we will see, _testing the bytecode compiler_
actually involves testing two compilers, and the same goes for _testing
the native compiler_.
...
ocamltest terminology. An _action_ is anything that can `pass`, `skip` or
...
4. `run`: now that the program has been successfully compiled,
it is run with its standard output and error streams saved to a file.
...
8. `check-ocamlc.opt-output`: again, this action is similar to action 3.
...
thus of tests) may depend on _variables_ whose value can be adjusted in
...
_environment modifiers_. As will be explained in <<concepts>>, an
...
In addition, the script can write lines of the form `variable="string"`, or
`variable+="string"`, or `-variable`, to file `${ocamltest_response}`, to set, append to,
and unset ocamltest environment variables, respectively.
