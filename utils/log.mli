...
val log_version: _ t -> Diagnostic.version option
val log_scheme: 'id t -> 'id Diagnostic.t
...
val make:
  ...
  Diagnostic_validation.version -> 'a Diagnostic.t -> Device.t -> 'a t

val tmp: 'a Diagnostic.t -> 'a t
...
val set: 'id t -> ('a,'id, _) field -> 'a -> unit
val (.%[]<-): 'id t -> ('a,'id, _) field -> 'a -> unit
...
val cons: 'id t -> ('a list, 'id, _) field -> 'a -> unit
...
val redirect: 'id t -> ('a,'id,_) field -> Device.t -> unit
...
val detach: 'id t -> ('id2 Diagnostic.record, 'id, _) field -> 'id2 t
...
val detach_item:
  'id t -> ('id2 Diagnostic.record list, 'id, _) field -> 'id2 t

(** {1:log_non_streaming Non-streaming log contents} *)

val get: 'id t -> ('a,'id, _) field -> 'a option
val dynamic_get: 'id t -> string -> Diagnostic.typed_val option

(** [replay source dest] transfer the contents of the [source] log (if any) to
    the [dest] log.*)
val replay: source:'a t -> dest:'a t -> unit

(** {1 Printing functions }*)

val f :
  (string,'a, _) field -> 'a t -> ('b, Format.formatter, unit) format -> 'b
  (** [fmt field log ppf] records the output of [ppf] as
      a string at field [field] in [log].
  *)

val d :
  (Format_doc.t,'a,_) field -> 'a t -> ('b, Format_doc.formatter, unit) format
  -> 'b
  (** [fmt field log ppf] records the formatted message at field [field] in
      [log]. *)

val itemf :
  (string list,'a, _) field -> 'a t -> ('b, Format.formatter, unit) format ->
  'b

val itemd :
  (Format_doc.t list,'a,_) field -> 'a t
  -> ('b, Format_doc.formatter, unit) format -> 'b

val log_if:
  'id t -> (string, 'id, _) field -> bool ->
