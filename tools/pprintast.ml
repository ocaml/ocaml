...
(* should this even print by itself? *)
  ...
  | Pexp_open (lid, e) ->
      ...
      pp_close_box ppf () ;
(*>JOCAML *)
  | Pexp_spawn e -> (* Cf. lazy *)
      pp_open_hovbox ppf indent ;
      fprintf ppf "spawn@ " ;
      simple_expr ppf e ;
      pp_close_box ppf ()
  | Pexp_par (e1,e2) -> (* Cf. sequence *)
      fprintf ppf "@[<hv 0>(" ;
      pp_print_break ppf 1 indent ;
      expression_par ppf ~first:false x ;
      fprintf ppf "@;<1 0>)@]"
  | Pexp_reply (e,id) -> (* Cf. lazy *)
      pp_open_hovbox ppf indent ;
      fprintf ppf "reply@ " ;
      simple_expr ppf e ;
      fprintf ppf "@ to %s" id.txt ;
      pp_close_box ppf ()
  | Pexp_def (d,e) -> (* Cf. let *)
      let d,ds = Misc.as_cons d in
      pp_open_hvbox ppf 0 ;
      pp_open_hvbox ppf indent ;
      fprintf ppf "def " ;
      auto ppf d ;
      ...
      fprintf ppf " in" ;
      pp_print_space ppf () ;
      expression_par ppf ~first:false ~indent:0 e ;
      pp_close_box ppf () ;
(*<JOCAML *)
  | _ -> simple_expr ppf x

(*<JOCAML *)
and expression_par ppf ?(skip=1) ?(indent=indent) ?(first=true) expr =
  if (first = true) then begin
    pp_open_hvbox ppf 0 ;
    expression_par ppf ~skip:skip ~indent:0 ~first:false expr ;
    pp_close_box ppf () ;
  end else
    ...
    | Pexp_par (e1, e2) ->
         ...
         expression_par ppf ~skip:skip ~indent:indent ~first:false e2 ;
    | _ ->
         expression ppf expr
(*>JOCAML *)
...
(*>JOCAML *)
    | Pstr_exn_global li ->
        pp_open_hovbox ppf indent ;
        fprintf ppf "def exception %a" fmt_longident li ;
        pp_close_box ppf () ;
    | Pstr_def ds -> (* Imitate let *)
        let d,ds = Misc.as_cons ds in
        pp_open_hvbox ppf 0 ;
        pp_open_hvbox ppf indent ;
        fprintf ppf "def " ;
        auto ppf d ;
...
and jpat ppf jp =
  let jid,p = jp.pjpat_desc in
  fprintf ppf "%s" jid.txt ;
  pattern_in_parens ppf p

and clause ppf cl =
  let jps,e = cl.pjclause_desc in
  let jp,jps = Misc.as_cons jps in
  fprintf ppf "@[<hov 1>";
  list2 jpat ppf jps "&" ;
  fprintf ppf "@])";
  fprintf ppf " =@ " ;
  expression_par ppf e ~first:false

(* Same trick as in  pattern_x_expression_def_list *)
and clause_list ppf cls = match cls with
| [] -> ()
| cl::cls ->
    pp_close_box ppf () ;
    pp_print_space ppf () ;
...
and auto ppf a =
  let cls = a.pjauto_desc in
  let cl,cls = Misc.as_cons cls in
  pp_open_hvbox ppf 0 ;
  clause ppf cl ;
  clause_list ppf cls ;
  pp_close_box ppf ()

and autos ppf ds = match ds with
| [] -> ()
| a::ds ->
    pp_close_box ppf () ;
    pp_print_space ppf () ;
