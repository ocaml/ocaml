...
module Options = Main_args.Make_optcomp_options (struct
  ...
  let _dwarf_format s = option_with_int "-dwarf-format" s
  let _dwarf_version s = option_with_arg "-dwarf-version" s
  let _dwarf_offsets = option "-dwarf-offsets"
  let _no_dwarf_offsets = option "-no-dwarf-offsets"
  let _dwarf_emit_self_tail_calls = option "-dwarf-emit-self-tail-calls"
  let _no_dwarf_emit_self_tail_calls = option "-no-dwarf-emit-self-tail-calls"
  ...
  let _drawclambda_dbg = option "-drawclambda-dbg"
  let _dclambda = option "-dclambda"
  let _dclambda_dbg = option "-dclambda-dbg"
  let _dmach_dbg = option "-dmach-dbg"
  let _dlinear_dbg = option "-dlinear-dbg"
  ...
  let _ddebug_invariants = option "-ddebug-invariants"
  let _dno_debug_invariants = option "-dno-debug-invariants"
  ...
  let _gjs_of_ocaml = option "-gjs_of_ocaml"
  let _gno_js_of_ocaml = option "-gno_js_of_ocaml"
  let _gocamldebug = option "-gocamldebug"
  let _gno_ocamldebug = option "-gno_ocamldebug"
  let _gsubprocs = option "-gsubprocs"
  let _gno_subprocs = option "-gno_subprocs"
  let _gbacktraces = option "-gbacktraces"
  let _gno_backtraces = option "-gno_backtraces"
  let _gbounds_checking = option "-gbounds_checking"
  let _gno_bounds_checking = option "-gno_bounds_checking"
  let _gdisable_bytecode_opt = option "-gdisable_bytecode_opt"
  let _gno_disable_bytecode_opt = option "-gno_disable_bytecode_opt"

  let _gdwarf_format = option_with_int "-gdwarf_format"
  let _gdwarf_version = option_with_string "-gdwarf_version"

  let _gdwarf_cfi = option "-gdwarf_cfi"
  let _gno_dwarf_cfi = option "-gno_dwarf_cfi"
  let _gdwarf_loc = option "-gdwarf_loc"
  let _gno_dwarf_loc = option "-gno_dwarf_loc"
  let _gdwarf_scopes = option "-gdwarf_scopes"
  let _gno_dwarf_scopes = option "-gno_dwarf_scopes"
  let _gdwarf_vars = option "-gdwarf_vars"
  let _gno_dwarf_vars = option "-gno_dwarf_vars"
  let _gdwarf_call_sites = option "-gdwarf_call_sites"
  let _gno_dwarf_call_sites = option "-gno_dwarf_call_sites"
  let _gdwarf_cmm = option "-gdwarf_cmm"
  let _gno_dwarf_cmm = option "-gno_dwarf_cmm"

  let _gdwarf_offsets = option "-gdwarf_offsets"
  let _gno_dwarf_offsets = option "-gno_dwarf_offsets"
  let _gdwarf_self_tail_calls = option "-gdwarf_self_tail_calls"
  let _gno_dwarf_self_tail_calls = option "-gno_dwarf_self_tail_calls"
