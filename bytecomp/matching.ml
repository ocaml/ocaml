  ...
  | (_,act0) :: rem ->
      try
        ...
        let rec s_rec = function
          | [] -> Some act0
          | (_,act)::rem ->
              if Lambda.same raw_act0 (raw_rec [] act) then
                s_rec rem
              else
                None in
        s_rec rem
...
let do_for_multiple_match loc paraml pat_act_list partial =
  ...
  try
    try
      ...
      let dummy_arg = { l_loc = loc; l_desc = Lconst (Const_pointer 0) } in
      let lam, total =
        comp_match_handlers
