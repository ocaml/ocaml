...
exception Not_compatible_32 of (string * string)
...
let marshal_to_channel_with_possibly_32bit_compat ~filename ~kind outchan obj =
  ...
  with Failure _ ->
    raise (Not_compatible_32 (filename, kind))
...
let () =
  Location.register_error_of_exn
    (function
      | Not_compatible_32 info -> Some (Location.error_of_printer_file report_error info)
      | _ -> None
    )
