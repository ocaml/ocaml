...
and transl_exp0 e =
  ...
  | Texp_setfield(arg, _, lbl, newval) ->
      let access =
        match lbl.lbl_repres with
          Record_regular
        | Record_inlined _ -> Psetfield(lbl.lbl_pos, Ctype.maybe_pointer newval)
        | Record_float -> Psetfloatfield lbl.lbl_pos
        | Record_extension -> Psetfield (lbl.lbl_pos + 1, Ctype.maybe_pointer newval)
      in
  ...
  then begin
    ...
    let update_field (_, lbl, expr) cont =
      let upd =
        match lbl.lbl_repres with
          Record_regular
        | Record_inlined _ -> Psetfield(lbl.lbl_pos, Ctype.maybe_pointer expr)
        | Record_float -> Psetfloatfield lbl.lbl_pos
        | Record_extension -> Psetfield(lbl.lbl_pos + 1, Ctype.maybe_pointer expr)
      in
