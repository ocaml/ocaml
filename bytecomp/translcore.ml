...
let transl_primitive loc p =
  ...
  match prim with
    Plazyforce ->
      ...
      mk_lam (Lfunction(Curried, [parm], Matching.inline_lazy_force (mk_lam (Lvar parm)) Location.none))
  | _ ->
      let rec make_params n =
        if n <= 0 then [] else Ident.create "prim" :: make_params (n-1) in
      let params = make_params p.prim_arity in
      mk_lam (Lfunction(Curried, params, mk_lam (Lprim(prim, List.map (fun id -> mk_lam (Lvar id)) params))))
...
let assert_failed exp =
  let (fname, line, char) = Location.get_pos_info exp.exp_loc.Location.loc_start in
    let make = mk_loc_lam exp.exp_loc in
    make
      (Lprim(Praise,
...
and transl_apply lam sargs loc =
  ...
  let rec build_apply lam args = function
      (None, optional) :: l ->
        ...
        let body =
          let app = build_apply handle ((mk_lam (Lvar id_arg), optional)::args') l in
          mk_loc_lam app.l_loc
            begin match app.l_desc with
              ...
              | Levent({l_desc=Lfunction(Curried, ids, lam)}, _) ->
                Lfunction(Curried, id_arg::ids, lam)
              | _ ->
                Lfunction(Curried, [id_arg], app)
            end in
        List.fold_left
          (fun body (id, lam) -> mk_lam (Llet(Strict, id, lam, body)))
          body !defs
    | (Some arg, optional) :: l ->
  ...
  then begin
    ...
    | Some init_expr ->
        for i = 0 to Array.length all_labels - 1 do
          ...
          lv.(i) <- mk_lam (Lprim(access, [mk_lam (Lvar init_id)]))
        done
    end;
    List.iter
    ...
    begin match opt_init_expr with
      None -> lam
    | Some init_expr -> mk_lam (Llet(Strict, init_id, transl_exp init_expr, lam))
    end
  end else begin
    ...
    let rec update_field (_, _, lbl, expr) cont =
      let upd =
        ...
        | Record_float -> Psetfloatfield lbl.lbl_pos in
      mk_lam
        (Lsequence(mk_lam (Lprim(upd, [mk_lam (Lvar copy_id); transl_exp expr])),
                   cont)) in
    begin match opt_init_expr with
      None -> assert false
    | Some init_expr ->
        mk_lam (Llet(Strict, copy_id,
                     mk_lam (Lprim(Pduprecord (repres, size), [transl_exp init_expr])),
                     List.fold_right update_field lbl_expr_list (mk_lam (Lvar copy_id))))
    end
  end
...
let transl_exception id path decl =
  ...
  mk_lam (Lprim(Pmakeblock(0, Immutable),
                [mk_lam (Lconst(Const_base(Const_string name)))]))
