...
let combine_constant arg cst partial ctx def 
  ...
  let lambda1 = switch_constant cst arg const_lambda_list fail in
...
let combine_constructor arg ex_pat cstr partial ctx def
  ...
  if cstr.cstr_consts < 0 then begin
    ...
    let cls = to_add@tag_lambda_list in
    let lambda1 = switch_exn arg cls fail in
    ...
    let cls = fails @ tag_lambda_list in
    let lambda1 = switch_constr cstr arg cls None in
    lambda1, jumps_union local_jumps total1
  end


let combine_variant row arg partial ctx def (tag_lambda_list, total1, pats) =
  let num_constr = count_variants row in
  let sig_complete =  List.length tag_lambda_list = num_constr in
  let fail, to_add, local_jumps =
    if
    ...
    then
      ...
      mk_failaction_neg partial ctx def in
  let cls = to_add@tag_lambda_list in
  let lambda1 = switch_variant arg cls fail in  
  lambda1, jumps_union local_jumps total1
...
let combine_array arg kind partial ctx def
  ...
  let fail, to_add, local_jumps = mk_failaction_neg partial  ctx def in
  let cls = to_add @ len_lambda_list in
  let lambda1 = switch_array kind arg cls fail in
  lambda1, jumps_union local_jumps total1
...
(* Once for checking that compilation is possible *)
      ...
      let args =  List.map (fun id -> Lvar id, StrictOpt) idl in
