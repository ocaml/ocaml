...
intnat caml_unix_nonblock_single_write(value fd, value buf, intnat ofs, intnat len)
{
  return write(Int_val(fd), &Byte(buf, ofs), len);
}

CAMLprim value caml_byte_unix_nonblock_single_write(value fd, value buf, value vofs,
                                   value vlen) {
  CAMLparam0();
