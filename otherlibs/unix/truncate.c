...
CAMLstub value unix_truncate(value path, value len)
{
  CAMLparam2(path, len);
  char * p;
  int ret;
...
CAMLstub value unix_truncate_64(value path, value vlen)
{
...
CAMLstub value unix_truncate_64(value path, value len)
{ caml_invalid_argument("truncate not implemented"); }
