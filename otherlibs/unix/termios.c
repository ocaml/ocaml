...
CAMLstub value unix_tcgetattr(value fd)
{
  value res;
...
CAMLstub value unix_tcsetattr(value fd, value when, value arg)
{
  if (tcgetattr(Int_val(fd), &terminal_status) == -1)
...
CAMLstub value unix_tcsendbreak(value fd, value delay)
{
...
CAMLstub value unix_tcdrain(value fd)
...
CAMLstub value unix_tcflush(value fd, value queue)
{
...
CAMLstub value unix_tcflow(value fd, value action)
{
