...
CAMLprim value unix_write_ba_1(value fd, value buf, value vofs, value vlen)
{
  CAMLparam0 ();
  long ofs, len, written;
  int numbytes, ret;
  ...
  while (len > 0) {
    numbytes = len > UNIX_BUFFER_SIZE ? UNIX_BUFFER_SIZE : len;
    caml_enter_blocking_section();
    ret = write(Int_val(fd), data + ofs, numbytes);
    caml_leave_blocking_section();
    if (ret == -1) {
      if ((errno == EAGAIN || errno == EWOULDBLOCK) && written > 0) break;
      uerror("write_bigstring", Nothing);
    }
    written += ret;
    ofs += ret;
    len -= ret;
...
CAMLprim value unix_single_write_ba_1(value fd, value buf, value ofs, value len)
{
  CAMLparam0 ();
  long ret, numbytes;
  char *data = (char *)Caml_ba_data_val(buf);

  numbytes = len > UNIX_BUFFER_SIZE ? UNIX_BUFFER_SIZE : Long_val(len);
  caml_enter_blocking_section();
  ret = write(Int_val(fd), data + Long_val(ofs), numbytes);
  caml_leave_blocking_section();
  if (ret == -1) uerror("single_write_bigstring", Nothing);
