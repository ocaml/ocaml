...
let extract_dll_name (~suffixed, file) =
  if not suffixed && Filename.check_suffix file Config.ext_dll then
    ...
    in
      if suffixed then
#80 "otherlibs/dynlink/byte/dynlink_symtable.ml"
...
let slot_for_getglobal global =
  ...
  with Not_found ->
#183 "otherlibs/dynlink/byte/dynlink_symtable.ml"
    let global =
      match global with
...
#261 "otherlibs/dynlink/byte/dynlink_symtable.ml"
