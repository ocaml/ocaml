...
9 |     with type t = M.t)
Error:
  This expression has type "(module S with type t = M.t)"
...
6 |     with type t = K.t)
Error:
  This expression has type "(module S with type t = A.t)"
...
15 |   unify ()
             ^^
Error:
  This expression has type "unit" but an expression was expected of type "M.t"
|}, Principal{|
module type S = sig type t val x : t end
