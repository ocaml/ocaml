...
let test x =
  ...
  let[@local] f4 x = 2 * x (* not local: it is used both inside r and
                              outside, no single handler can put both
                              in tail context. *) in
  let[@local] f5 x = f1 x (* local: tail position within r. *) in
  let[@local] f6 x = 3 * x (* ok: not in tail position, but its single
                              use can get inlined. *) in
  ignore (f2, f3);
  let r =
    if x = 1 then f1 x
...
(* TEST
 flags = "-no-g -w +A-70"; (* -g disables [@local] simplification *)
