...
let nid ?name () = Expr_id.create ?name ()
...
type env =
  { var : Variable.t Variable.Map.t }

let empty_env =
  { var = Variable.Map.empty }

let add_var v v' env =
  { var = Variable.Map.add v v' env.var }

let add_var_list l1 l2 env =
  List.fold_right2 add_var l1 l2 env

let equal_var env v v' =
  try Variable.equal (Variable.Map.find v env.var) v'
...
let add_same_var_set env s =
  Variable.Set.fold (fun v env -> add_var v v env) s env

let equal_set env s1 s2 =
  let img =
    Variable.Set.fold (fun v acc -> Variable.Set.add (Variable.Map.find v env.var) acc) s1 Variable.Set.empty in
  Variable.Set.equal img s2 &&
  Variable.Set.cardinal s1 = Variable.Set.cardinal s2
...
let rec equal env t1 t2 = match t1, t2 with
  ...
  | Fset_of_closures (c1, _), Fset_of_closures (c2, _) ->
      ...
      let env = add_same_var_set env (Variable.Map.keys c1.cl_free_var) in
      ...
      Variable.Map.equal (equal env) c1.cl_free_var c2.cl_free_var &&
