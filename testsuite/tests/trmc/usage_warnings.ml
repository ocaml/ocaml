...
3 |   | xs :: xss -> append xs (flatten xss)
                     ^^^^^^^^^^^^^^^^^^^^^^^
Warning 70: This call is in tail-modulo-cons position in a TMC function,
...
2 |   | [] -> []
3 |   | xs :: xss -> append xs (flatten xss)
Warning 69: This function is marked @tail_mod_cons but is never applied in TMC position.
