...
Warning 18 [not-principal]:
  this type-based field disambiguation is not principal.
...
Warning 45 [open-shadow-label-constructor]:
  this open statement shadows the label x (which is later used)
...
5 | let _ = ( { M.x=0 } : u );;
                ^^^
Error:
  The field "M.x" belongs to the record type "M.t"
