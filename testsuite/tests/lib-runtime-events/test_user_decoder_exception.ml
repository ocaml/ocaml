...
let () =
  ...
  (* The decoder exception must not leave cursor_in_poll set: before this was
     ...
     rather than surfacing the decoder's own error. The failing event is not
     consumed, so the decoder runs again and raises again. *)
  ...
  | _ -> print_string "FAIL: second poll did not raise"
