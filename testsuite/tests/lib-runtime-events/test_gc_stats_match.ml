...
(* This test verifies that runtime event counters match the GC statistics
   ...
   The child is run in two configurations: the default one, where the two
   must agree exactly, and with cleanup_on_exit (c=1), where the runtime
   collects again during shutdown after the statistics have been printed, so
   the counters must be at least the printed values. *)
...
let run_and_check ~exact child_prog cwd label runparam =
  ...
  (* The statistics are printed by caml_do_exit. With [cleanup_on_exit] the
     runtime then performs a final round of collections in
     caml_domain_terminate, and those are recorded in the ring, so the
     counters are ahead of the printed statistics by exactly that work. In
     the default mode nothing runs after the statistics are printed and the
     two must agree exactly. *)
  let check name gc_val ev_val =
    let ok = if exact then gc_val = ev_val else ev_val >= gc_val in
...
let () =
  ...
  let ok_default =
    run_and_check ~exact:true child_prog cwd "default" "v=0x400" in
  let ok_cleanup =
    run_and_check ~exact:false child_prog cwd "cleanup_on_exit" "v=0x400,c=1" in
