(* TEST
 ...
 not target-windows;
 {
   ...
   ocamlc.byte;
   program = "${test_build_directory}/test_gc_stats_match_cleanup.byte";
   all_modules = "test_gc_stats_match_cleanup.ml";
   ocamlc.byte;
   ...
   all_modules = "gc_stats_child.ml";
   ocamlopt.byte;
   program = "${test_build_directory}/test_gc_stats_match_cleanup.exe";
   all_modules = "test_gc_stats_match_cleanup.ml";
   ocamlopt.byte;
...
(* This test verifies that runtime event counters match the GC statistics
   ...
   This is a variant of test_gc_stats_match.ml that additionally sets
   cleanup_on_exit (c=1), so that the runtime shuts down gracefully on exit.

   It spawns a child process that:
   ...
   3. Compares then with the expectation they should match exactly.
*)
...
let () =
  ...
  if pid = 0 then begin
    ...
    (* Build environment with OCAMLRUNPARAM=v=0x400 for v=0x400 stats output,
       and c=1 to enable cleanup_on_exit. *)
    let env = Unix.environment () |> Array.to_list in
    let env = List.filter (fun s ->
      not (String.length s >= 14 && String.sub s 0 14 = "OCAMLRUNPARAM=")
    ) env in
    let env = ("OCAMLRUNPARAM=v=0x400,c=1") :: env in
  ...
  let check name gc_val ev_val =
    if gc_val <> ev_val then begin
      Printf.printf "MISMATCH %s: v=0x400=%d events=%d\n" name gc_val ev_val;
      false
    end else
      true
  in
  ...
  let ok =
    ...
    && check "minor_collections" gc_stats.minor_collections ev.ev_minor_collections
    && check "major_collections" gc_stats.major_collections ev.ev_major_collections
  in
