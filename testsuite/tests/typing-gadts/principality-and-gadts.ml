...
Warning 18 [not-principal]:
  typing this pattern requires considering int and a as equal.
...
5 |   | BoolLit, b -> 1
        ^^^^^^^
Warning 18 [not-principal]:
  typing this pattern requires considering bool and a as equal.
But the knowledge of these types is not principal.
...
4 |   | MAB -> false;;
        ^^^
Warning 18 [not-principal]:
  typing this pattern requires considering unit M.mab and unit ab as equal.
But the knowledge of these types is not principal.
...
4 |   | AB -> true
        ^^
Warning 18 [not-principal]:
  typing this pattern requires considering unit ab and x as equal.
But the knowledge of these types is not principal.
...
5 |   | MAB -> false;;
        ^^^
Warning 18 [not-principal]:
  typing this pattern requires considering unit M.mab and x as equal.
But the knowledge of these types is not principal.
...
3 |    let Refl = e in x;;
           ^^^^
Warning 18 [not-principal]:
  typing this pattern requires considering x and int option as equal.
But the knowledge of these types is not principal.
...
3 |   | [ { a = 3; _ } ; { b = F; _ }] -> ()
                               ^
Warning 18 [not-principal]:
  typing this pattern requires considering Foo.t and int as equal.
But the knowledge of these types is not principal.
|}]
...
3 |   | [ { a = 3; _ }; { b = Refl3 ; _ }] -> ()
                              ^^^^^
Warning 18 [not-principal]:
  typing this pattern requires considering int and Foo.t as equal.
But the knowledge of these types is not principal.
|}]
...
3 |   | { x = x; eq = Refl3 } -> x
                      ^^^^^
Warning 18 [not-principal]:
  typing this pattern requires considering M.t and N.t as equal.
But the knowledge of these types is not principal.
...
3 |   | { x = (x : N.t); eq = Refl3 } -> x
        ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error:
  This pattern matches values of type "N.t foo"
