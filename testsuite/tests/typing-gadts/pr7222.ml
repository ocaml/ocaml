...
Line 9, characters 11-18:
    let Cons(Elt dim, _) = sh in ()
             ^^^^^^^
Error:
This pattern matches values of type ($Cons_'x, 'a -> $Cons_'x) elt
...
Line 9, characters 6-22:
    let Cons(Elt dim, _) = sh in ()
        ^^^^^^^^^^^^^^^^
Error:
This pattern matches values of type ('a -> $0 -> nil) t
but a pattern was expected which matches values of type ('a -> 'b -> nil) t
