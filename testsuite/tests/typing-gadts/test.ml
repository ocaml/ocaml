...
Line 5, characters 21-22:
      let eval (D x) = x
                       ^
Error:
This expression has type $D_'a t but an expression was expected of type 'a
...
Line 9, characters 11-19:
          | (IntLit _ | BoolLit _) -> ()
             ^^^^^^^^
Error:
This pattern matches values of type int t
...
Line 2, characters 14-15:
    fun Eq o -> o ;; (* fail *)
                ^
Error:
This expression has type [> `A of a ] but an expression was expected of type
  [> `A of b ]
Type a is not compatible with type b = a
This instance of a is ambiguous: it would escape the scope of its equation
|}, Principal{|
Line 2, characters 9-15:
    fun Eq o -> o ;; (* fail *)
           ^^^^^^
Error:
This expression has type ([> `A of b ] as 'a) -> 'a
...
Line 1, characters 4-84:
  ....f : type a b. (a,b) eq -> [< `A of a | `B] -> [< `A of b | `B] =
    fun Eq o -> o..............
Error:
This definition has type ('a, 'b) eq -> ([< `A of 'b & 'a | `B ] as 'c) -> 'c
