...
Error:
  ...
  The type "< m : int; n : int >" is not equal to the type
    "< m : float * int; n : int >"
  The method "m" has type "int", but the expected method type was "float * int"
|}];;
...
Error:
  ...
  The type "[> `A of int ]" is not equal to the type "[> `A of int | `B of int ]"
...
Error:
  ...
  is not included in
    val f : s -> s
  The type "< m : int > -> < m : int >" is not compatible with the type "s -> s"
...
Error:
  ...
  The method "m" has type "'b. [< `Foo ] as 'b", but the expected method type was
  "[< `Foo ]"
|}];;
...
Error:
  ...
  is not included in
    val f : [< `C ] -> unit
  The type "[< `C of & int & float ] -> unit" is not compatible with the type
    "[< `C ] -> unit"
  Types for tag "`C" are incompatible
|}];;
...
Error:
  Signature mismatch:
  Modules do not match:
    sig type _ t = A : (< x : 'b * 'b > as 'b) -> (< y : 'a > as 'a) t end
  is not included in
  ...
  The type "< x : 'a * 'a > as 'a" is not equal to the type "< x : 'b > as 'b"
...
Error:
  ...
  is not included in
    sig type _ ext += A : (< x : 'b > as 'b) -> (< y : 'a > as 'a) ext end
