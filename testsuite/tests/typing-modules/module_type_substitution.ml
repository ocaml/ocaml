...
1 | module type fst_erased = fst with module type t := sig end
                             ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error:
  This "with" constraint "t := sig end" makes a packed module ill-formed.
...
8 | module type R = S with module type M.T := sig end
                    ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error:
  This "with" constraint "M.T := sig end" makes a packed module ill-formed.
  (see manual section 12.7.3)
|}]
...
3 |   val x: (module t)
      ^^^^^^^^^^^^^^^^^
Error:
  The module type "t" is not a valid type for a packed module:
