...
2 | module M = F(struct type x end)
               ^^^^^^^^^^^^^^^^^^^^
Error:
  Modules do not match: sig type x end
is not included in sig type t end The type "t" is required but not provided
|}]
...
4 | type u = F(X)(Z).t
             ^^^^^^^^^
Error:
  The functor application "F(X)(Z)" is ill-typed.
  These arguments:
    X Z
  do not match these parameters:
    functor (X : ...) (Y : $T2) (Z : ...) -> ...
...
2 | module M = Map.Make(Ord)(Ord)
               ^^^^^^^^^^^^^^^^^^
Error:
  This application of the functor "Map.Make" is ill-typed.
  These arguments:
    Ord Ord
  do not match these parameters:
...
6 |     type t = F(X).t
                 ^^^^^^
Error:
  Modules do not match: a
is not included in a/2
    ...
    sig
      module F :
        functor
          ...
          (Z : functor (A : sig type za end) (B : sig type zb end) -> sig end)
          -> sig end
    end
  In module "F":
  Modules do not match:
              ...
              module D :
                sig
                  module E :
                    sig
                      module F :
                        functor (X : sig type x end) (Y : sig type y' end)
...
14 |   type u = G(X)(Y)(X)(Y)(X).t
                ^^^^^^^^^^^^^^^^^^
Error:
  The functor application "G(X)(Y)(X)(Y)(X)" is ill-typed.
  These arguments:
    A.X A.Y A.X A.Y A.X
  do not match these parameters:
...
Error:
  ...
  1. The following extra argument is provided
         Add_one' : sig module M = Add_one'.M module type t = Add_one'.t end
...
Error:
  ...
  Modules do not match:
    sig
      module F : functor (X : sig type x end) (Z : sig type z end) -> sig end
    end
  is not included in
    sig
      module F :
...
1 | type broken1 = Bar(B)(FiveArgsExt)(B)(AExt).a
                   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error:
  The functor application "Bar(B)(FiveArgsExt)(B)(AExt)" is ill-typed.
  These arguments:
...
1 | type broken2 = Bar(A)(FiveArgsExt)(TY)(TY)(TY)(TY)(TY).a
                   ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error:
  The functor application "Bar(A)(FiveArgsExt)(TY)(TY)(TY)(TY)(TY)" is ill-typed.
  These arguments:
...
2 | module R = F(struct end[@warning "-73"]);;
               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error:
  Modules do not match: sig end
is not included in A The type "a" is required but not provided
|}]
...
8 | module R = G(struct end)(struct let f (x,_) = x end)
               ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Error:
  This application of the functor "G" is ill-typed.
  These arguments:
    (struct end) $S2
  do not match these parameters:
    functor (A : $T1) (B : ...) -> ...
...
11 |     ()
Error:
  This application of the functor "With_expansion" is ill-typed.
  These arguments:
    $S1 () () ()
  do not match these parameters:
    functor (A : ...) (B : $T2) () () -> ...
...
25 |     end)
Error:
  This application of the functor "H" is ill-typed.
  These arguments:
    $S1 $S2
  do not match these parameters:
    functor (X : $T1) (Y : $T2) -> ...
  ...
  2. Modules do not match:
     ...
     is not included in
       val f : 'a X.s -> 'a
     The type "'a X.s -> 'a X.s" is not compatible with the type "'a X.s -> 'a"
...
1 | let f (x:Set.Make(Set)(A).t) = x
             ^^^^^^^^^^^^^^^^^^
Error:
  The functor application "Set.Make(Set)(A)" is ill-typed.
  These arguments:
    Set A
  do not match these parameters:
