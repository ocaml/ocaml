    ...
    (catch
      (catch
        (if (!= *match*/279 3) (exit 6)
          (let
            (x/282 : [0: int, int, int] =a
               (makeblock 0 *match*/278 *match*/279 *match*/280))
            (exit 4 x/282)))
       with (6)
        (if (!= *match*/278 1) (exit 5)
          (let
            (x/281 : [0: int, int, int] =a
               (makeblock 0 *match*/278 *match*/279 *match*/280))
            (exit 4 x/281))))
     with (5) 0)
   with (4 x/276[0: int, int, int]) (seq (ignore x/276) 1)))
(let (*match*/278 = 3 *match*/279 = 2 *match*/280 = 1)
  (catch
    (if (!= *match*/279 3)
      (if (!= *match*/278 1) 0
...
(* outside, trivial *)
[%%expect {|
(function a/287[int] b/288 : [0: int, *]
  (let (p/289 : [0: int, *] =a (makeblock 0 a/287 b/288)) p/289))
(function a/287[int] b/288 : [0: int, *] (makeblock 0 a/287 b/288))
- : bool -> 'a -> bool * 'a = <fun>
|}]
...
(* inside, trivial *)
[%%expect{|
(function a/291[int] b/292 : [0: int, *]
  (let (p/293 : [0: int, *] =a (makeblock 0 a/291 b/292)) p/293))
(function a/291[int] b/292 : [0: int, *] (makeblock 0 a/291 b/292))
- : bool -> 'a -> bool * 'a = <fun>
|}];;
...
(* outside, simple *)
[%%expect {|
(function a/297[int] b/298 : [0: int, [0: int, *]]
  (let
    (x/299 : int =a a/297 p/300 : [0: int, *] =a (makeblock 0 a/297 b/298))
    (makeblock 0 (int,[0: int, *]) x/299 p/300)))
(function a/297[int] b/298 : [0: int, [0: int, *]]
  (makeblock 0 (int,[0: int, *]) a/297 (makeblock 0 a/297 b/298)))
...
(* inside, simple *)
[%%expect {|
(function a/303[int] b/304 : [0: int, [0: int, *]]
  (let
    (x/305 : int =a a/303 p/306 : [0: int, *] =a (makeblock 0 a/303 b/304))
    (makeblock 0 (int,[0: int, *]) x/305 p/306)))
(function a/303[int] b/304 : [0: int, [0: int, *]]
  (makeblock 0 (int,[0: int, *]) a/303 (makeblock 0 a/303 b/304)))
- : bool -> 'a -> bool * (bool * 'a) = <fun>
|}]
...
(* outside, complex *)
[%%expect{|
(function a/313[int] b/314[int] : [0: int, [0: int, int]]
  (if a/313
    (let
      (x/315 : int =a a/313
       p/316 : [0: int, int] =a (makeblock 0 a/313 b/314))
      (makeblock 0 (int,[0: int, int]) x/315 p/316))
    (let
      (x/317 : [0: ] =a b/314
       p/318 : [0: int, int] =a (makeblock 0 a/313 b/314))
      (makeblock 0 (int,[0: int, int]) x/317 p/318))))
(function a/313[int] b/314[int] : [0: int, [0: int, int]]
  (if a/313 (makeblock 0 (int,[0: int, int]) a/313 (makeblock 0 a/313 b/314))
    (makeblock 0 (int,[0: int, int]) b/314 (makeblock 0 a/313 b/314))))
...
(* inside, complex *)
[%%expect{|
(function a/319[int] b/320[int] : [0: int, [0: int, int]]
  (catch
    (if a/319
      (let
        (x/327 : int =a a/319
         p/328 : [0: int, int] =a (makeblock 0 a/319 b/320))
        (exit 10 x/327 p/328))
      (let
        (x/325 : [0: ] =a b/320
         p/326 : [0: int, int] =a (makeblock 0 a/319 b/320))
        (exit 10 x/325 p/326)))
   with (10 x/321[int] p/322[0: int, int])
    (makeblock 0 (int,[0: int, int]) x/321 p/322)))
(function a/319[int] b/320[int] : [0: int, [0: int, int]]
  (catch
...
(* outside, onecase *)
[%%expect {|
(function a/329[int] b/330[int] : [0: int, [0: int, int]]
  (if a/329
    (let
      (x/331 : int =a a/329
       _p/332 : [0: int, int] =a (makeblock 0 a/329 b/330))
      (makeblock 0 (int,[0: int, int]) x/331 [0: 1 1]))
    (let
      (x/333 : int =a a/329
       p/334 : [0: int, int] =a (makeblock 0 a/329 b/330))
      (makeblock 0 (int,[0: int, int]) x/333 p/334))))
(function a/329[int] b/330[int] : [0: int, [0: int, int]]
  (if a/329 (makeblock 0 (int,[0: int, int]) a/329 [0: 1 1])
    (makeblock 0 (int,[0: int, int]) a/329 (makeblock 0 a/329 b/330))))
- : bool -> bool -> bool * (bool * bool) = <fun>
|}]
...
(* inside, onecase *)
[%%expect{|
(function a/335[int] b/336 : [0: int, [0: int, *]]
  (let
    (x/337 : int =a a/335 p/338 : [0: int, *] =a (makeblock 0 a/335 b/336))
    (makeblock 0 (int,[0: int, *]) x/337 p/338)))
(function a/335[int] b/336 : [0: int, [0: int, *]]
  (makeblock 0 (int,[0: int, *]) a/335 (makeblock 0 a/335 b/336)))
- : bool -> 'a -> bool * (bool * 'a) = <fun>
|}]
...
(* outside, tuplist *)
[%%expect {|
(function a/348[int] b/349 : [0: int, *]
  (catch
    (if a/348 (if b/349 (let (p/350 =a (field 0 b/349)) p/350) (exit 12))
      (exit 12))
   with (12) (let (p/351 : [0: int, *] =a (makeblock 0 a/348 b/349)) p/351)))
(function a/348[int] b/349 : [0: int, *]
...
(* inside, tuplist *)
[%%expect{|
(function a/352[int] b/353 : [0: int, *]
  (catch
    (catch
      (if a/352
        ...
        (exit 14))
     with (14)
      (let (p/356 : [0: int, *] =a (makeblock 0 a/352 b/353))
        (exit 13 p/356)))
   with (13 p/354[0: int, *]) p/354))
(function a/352[int] b/353 : [0: int, *]
  (catch
    (catch
