...
5 |   let f: t -> t = fun B -> x
                               ^
Error:
  This expression has type "t/2" but an expression was expected of type "t"
...
7 |   let f : M.t -> M.t = fun M.C -> y
                                      ^
Error:
  This expression has type "M/2.t" but an expression was expected of type "M.t"
...
2 | let x: ttt = let rec y = A y in y;;
                                    ^
Error:
  This expression has type "ttt/2" but an expression was expected of type "ttt"
