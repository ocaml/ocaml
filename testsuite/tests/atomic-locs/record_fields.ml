...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Basic/356"
  (let
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Ok/383" (makeblock 0))
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Inline_record/391"
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Extension_with_inline_record/399"
  (let
    (A =
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Float_records/414"
  (let
...
(apply (field_mut 1 (global Toploop!)) "Pattern_matching_wildcard/434"
  (let
