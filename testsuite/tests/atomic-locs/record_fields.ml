...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Basic/340"
  (let
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Ok/367" (makeblock 0))
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Inline_record/375"
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Extension_with_inline_record/383"
  (let
    (A =
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Float_records/398"
  (let
...
(apply (field_mut 1 (global Toploop!)) "Pattern_matching_wildcard/418"
  (let
