...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Basic/347"
  (let
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Ok/374" (makeblock 0))
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Inline_record/382"
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Extension_with_inline_record/390"
  (let
    (A =
...
[%%expect{|
(apply (field_mut 1 (global Toploop!)) "Float_records/405"
  (let
...
(apply (field_mut 1 (global Toploop!)) "Pattern_matching_wildcard/425"
  (let
