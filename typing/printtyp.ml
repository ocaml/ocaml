...
module Doc = struct
  type formatter = Format_doc.formatter
...
module Compat = struct
  ...
  let prepared_type_expr = Fmt.compat prepared_type_expr
  let shared_type_scheme = Fmt.compat shared_type_scheme
  let prepared_type_scheme = Fmt.compat prepared_type_scheme
  let signature = Fmt.compat signature
  let printed_signature = Fmt.compat1 printed_signature
  ...
  let prepared_constructor = Fmt.compat prepared_constructor

  let type_declaration = Fmt.compat1 type_declaration
  let prepared_type_declaration = Fmt.compat1 prepared_type_declaration

  let extension_constructor = Fmt.compat1 extension_constructor
  let prepared_extension_constructor = Fmt.compat1 prepared_extension_constructor
  let extension_only_constructor = Fmt.compat1 extension_only_constructor

  (* we duplicate code here due to the higher-order type,
     where printers occur both in positive and negative positions,
     making compatiblity-wrappers difficult to write and reason about. *)
  let functor_parameters =
    let module Fmt = Format in
    let rec functor_parameters ~sep custom_printer = function
      | [] -> ignore
      | [id,param] ->
          Fmt.dprintf "%t%t"
  ...
  module Conflicts = struct
    include Conflicts
    let print_located_explanations = Fmt.compat print_located_explanations
  end

  let report_ambiguous_type_error
    ppf env pair pairs a b c
    =
    Fmt.compat0 (fun fmt ->
      report_ambiguous_type_error fmt env pair pairs a b c
    ) ppf

  let report_unification_error
    ppf env err ?type_expected_explanation a b
    =
    Fmt.compat0 (fun fmt ->
      report_unification_error
        fmt env err ?type_expected_explanation a b
    ) ppf
