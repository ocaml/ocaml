...
let constructor_descriptions ty_res cstrs priv =
...
let exception_descr path_exc (ename, ty_list) =
  { cstr_name = ename;
    cstr_res = Predef.type_exn;
    cstr_args = ty_list;
    cstr_arity = List.length ty_list;
...
let label_descriptions ty_res lbls repres priv =
    ...
    | (name, mut_flag, ty_arg) :: rest ->
        let ldesc =
          { lbl_name = name;
            ...
            lbl_repres = repres;
            lbl_private = priv } in
        all_labels.(num) <- ldesc;
        ldesc :: describe_labels (num + 1) rest in
...
let find_constr_by_tag tag cstrs =
  find_constr tag 0 0 cstrs
