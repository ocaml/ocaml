...
and mcomp_row type_pairs env row1 row2 =
  ...
  in
  let row1_closed = row_closed row1 and row2_closed = row_closed row2 in
  if row1_closed && List.exists cannot_erase r2
  || row2_closed && List.exists cannot_erase r1
  (* correct condition if empty polymorphic variants are not allowed *)
  || pairs = [] && row1_closed && row2_closed
  then raise Incompatible;
  List.iter
    (fun (_,f1,f2) ->
