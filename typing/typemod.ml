...
and type_structure ?(toplevel = false) ~funct_body anchor env sstr =
  ...
  let rec type_struct env shape_map sstr =
    ...
    | pstr :: srem ->
        let previous_saved_types = Cmt_format.get_saved_types () in
        let str, sg, shape_map, new_env = type_str_item ~names ~toplevel ~funct_body anchor env shape_map pstr in
...
and type_str_item ~names ~toplevel ~funct_body anchor env shape_map {pstr_loc = loc; pstr_desc = desc} =
...
let type_str_item env pstri =
  let si, _, _, new_env =
    type_str_item ~toplevel:false ~funct_body:false ~names:(Signature_names.create ())
      None env Shape.Map.empty pstri in
  si, new_env
