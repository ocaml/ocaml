...
module EnvTbl =
  struct
    ...
    let map f (tbl,slots) = Ident.map f tbl,slots
  end
...
(*>JOCAML *)
and store_cont id path desc env =
   { env with
    continuations = EnvTbl.add id (path, desc) env.continuations; }
(* What is summary for ??? *)
(*<JOCAML *)
...
and add_continuation id desc env  =
  ...
  store_cont id (Pident id) desc env
