...
module type Printers := sig
  ...
  type 'a printer' := 'a -> formatter -> unit
  type doc
  ...
  *)
  val functor_parameters:
    sep:unit printer -> 'b printer' -> (Ident.t option * 'b) list printer'
  ...
  val report_unification_error :
    ...
    ?type_expected_explanation:doc -> doc -> doc ->
    unit
  ...
  val printed_signature: string -> signature printer
end

module Doc : Printers with type formatter = Format_doc.formatter and type doc = Format_doc.Doc.t

(* compatibility mode for old-style format printers *)
module Compat : Printers with type formatter = Format.formatter

(* include compatibility mode at the root of the module
   to avoid breaking third-party compiler-libs code. *)
