...
and type_expect_
  ...
  | Pexp_open (ovf, me, e) -> begin
      ...
      | Some {Ident.name} ->
        let pmod = Pmod_ident ({txt=Longident.Lident name; loc}) in
        ...
        let sexp = {pexp_desc; pexp_loc=loc;
          pexp_attributes=sexp.pexp_attributes} in
