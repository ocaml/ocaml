...
and type_expect_
  ...
  | Pexp_let (rec_flag, spat_sexp_list, sbody) ->
      let pat_exp_list, body =
        ...
        | Recursive ->
            ...
            let pat_exp_list = annotate_recursive_bindings env pat_exp_list in
            (pat_exp_list, body)
        | Nonrecursive ->
            let existential_context =
              if List.compare_length_with spat_sexp_list 1 > 0 then In_group
  ...
  | Pexp_constraint (sarg, spt) ->
      ...
      rue {
        ...
        exp_loc = arg.exp_loc;
        exp_type = instance_scheme ~keep_names:true ty;
...
and do_relaxed_value_restriction env pat_list exp_list =
  List.iter2
    (fun (pat, _) (exp, vars) ->
...
(* Adds the pattern variables [pvs] to env.
   Additionally binds any type vars used in the patterns. *)
and add_let_pattern_vars env ~pvs ~bind_type_vars_delayed =
  let new_env = add_pattern_variables env pvs in
  List.iter (fun f -> f ()) bind_type_vars_delayed;
  new_env

and check_let_univars env pat_list exp_list =
  List.iter2
