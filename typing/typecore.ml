...
module Params_suffix: sig

  (** List of syntactic parameters with additional metadata *)
  type t
  val make: Parsetree.function_param list -> Parsetree.function_body -> t

  val uncons: t -> (Parsetree.function_param * t) option
  val no_unlabelled_ahead: t -> bool

  (** The location generated by [mk_loc] and [loc] extends from the
      start of the first parameter to the end of the overall
      function. The parser does not construct such a location so we
      forge one for type errors.  *)
  val mk_loc:
    loc:Location.t -> first:bool -> Parsetree.function_param list
    -> Parsetree.function_body -> Location.t
  val loc: loc_function:Location.t -> first:bool -> t
    -> Parsetree.function_body -> Location.t
end = struct

  type t = {
    unlabelled_pos: int;
    (** relative position of the last unlabelled parameter.
        Special values:
        - [Int.max_int]: the function body is a `function .. | .. -> ...
        - negative values mean that there is no unlabelled parameter remaining
          in the list of parameters. *)

    suffix: Parsetree.function_param list;
  }

  let mk_loc
      ~(loc : Location.t) ~first params_suffix body : Location.t
    =
    match params_suffix, body with
    | { pparam_loc } :: _, _ ->
        if first then loc else
          let loc_start = pparam_loc.loc_start in
          { loc_start; loc_end = loc.loc_end; loc_ghost = true }
    | [], Pfunction_body pexp -> pexp.pexp_loc
    | [], Pfunction_cases (_, loc_cases, _) -> loc_cases

  let make suffix body =
    match body with
    | Pfunction_cases _ -> { unlabelled_pos = Int.max_int; suffix }
    | _ ->
        let f (current,last) arg = match arg.pparam_desc with
          | Pparam_newtype _ | Pparam_val ((Optional _|Labelled _),_,_) ->
              current + 1, last
          | Pparam_val(Nolabel,_,_) ->
              current + 1, current
        in
        let _, last = List.fold_left f (1,-1) suffix in
        { suffix; unlabelled_pos = last }

  let loc ~loc_function:loc ~first x body = mk_loc ~loc ~first x.suffix body
  let uncons x = match x.suffix with
    | [] -> None
    | param :: suffix ->
        Some (param, { suffix; unlabelled_pos = pred x.unlabelled_pos})

  let no_unlabelled_ahead x = x.unlabelled_pos < 0
end
...
and type_approx_function env params c body ty_expected ~in_function ~first =
  let loc = Params_suffix.mk_loc ~first ~loc:(fst in_function) params body in
...
and type_expect_
  ...
  | Pexp_function (params, body_constraint, body) ->
      ...
      let params = Params_suffix.make params body in
...
and type_function
  ...
  =
  let ty_fun, loc_function = in_function in
  let loc = Params_suffix.loc ~loc_function ~first params_suffix body in
  match Params_suffix.uncons params_suffix with
  | Some ({ pparam_desc = Pparam_newtype newtype; pparam_loc = _ }, rest) ->
  ...
  | Some ({ pparam_desc = Pparam_val (arg_label, None, pat); pparam_loc }, rest)
    ...
    ->
      ...
      *)
      ...
      (* This is quadratic, as it extracts all of the parameters from an arrow
         type for each optional parameter that's added. Now that functions are
         n-ary, this function is only used on syntactically-unerasable
         parameters but there might be an opportunity to improve this further.
      *)
      ...
      in
      (* An optional argument [?x] is syntactically-erasable if it is
         followed by a non-labelled argument in the current abstraction.

         In general, the argument  is only erasable if the function's return
      ...
      *)
      let unerasable_optional_arg ~syntactic =
        let warn =
          if syntactic then Warnings.Syntactic_unerasable_optional_argument
          else Warnings.Unerasable_optional_argument
        in
        Location.prerr_warning pat.pat_loc warn
      in
      (if is_optional arg_label
       && Params_suffix.no_unlabelled_ahead rest then
        if Warnings.is_active Unerasable_optional_argument then
          begin match only_labels_function_ret_tvar ty_ret with
          | Some true ->
              ...
              add_delayed_check (fun () ->
                  let syntactic =
                    only_labels_function_ret_tvar ty_ret <> Some false
                  in
                  unerasable_optional_arg ~syntactic
                )
          | Some false -> unerasable_optional_arg ~syntactic:false
          | None -> unerasable_optional_arg ~syntactic:true
          end
          else unerasable_optional_arg ~syntactic:true
      );
