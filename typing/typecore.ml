...
type pattern_checking_mode =
  ...
  (** We are checking a counter-example to exhaustivity
      generated by the pattern-matching analyser. *)


(** In [Counter_example] mode the responsibility of [type_pat] is to
   check that a counter-example to exhaustivity is valid: it has to be
   type-correct in a slightly different sense than for user-code, see
   [splitting_mode] below, and also capture at least one concrete
   value.
...
and counter_example_checking_info = {
    ...
    synthetic_names: counter_example_synthetic_names;
  }


(** For a pattern to be a valid counter-example, it must be
   inhabited -- match at least one value. A pattern may fail to match
   a value if some of its wildcard are at a type that is empty
   (typically due to incompatible GADT equations, although we also
   support empty sum types).

   The type-checker tries to figure out if a wildcard is in fact empty
   by "exploding" it, doing a case-analysis on possible
   constructors. It only tries up to a fixed depth bound, to avoid
   non-termination (this is a generic proof-search problem) or
   compilation-time slowdowns.

   [explosion_fuel] is this depth limit on wildcard explosion. *)
and explosion_fuel = int


(** A counter-example pattern may not be well-typed because the
   generator does not know about GADTs. In particular, it may generate
   (even in depth) disjunctions of the form

         (p1 | p2 | ... | pn),

   where some of the pi may in fact be ill-typed. If at least one of
   them is well-typed, we do have a valid counter-example. So we want
   the type-checker to succeed if at least one of the alternatives is
   well-typed.

   Note that the choice of value in such an or-pattern can influence
   the validity of other parts of the counter-example: if the
   alternatives are GADT constructors, each one may add equations that
   restrict other part of the counter-example in its own way. As
   a consequence, evaluating each choice may require arbitrary
   search/filtering in other parts of the counter-example (in
   implementation terms, the continuation [k] is called on each
   alternative).

   Searching after each alternative in this way may be very costly. It
   is only useful when GADTs are involved inside the or-pattern; if no
   GADT constructors occur (possibly deep) inside an alternative, it
   does no restrict the rest of the search space, so we can go with
   this alternative and never try the others. But we don't know
   whether a GADT constructor will occur (we haven't checked the
   pattern yet!).

   A possible implementation is to optimistically assume that
   splitting will not be necessary, and split only if we hit a GADT
   under an or-pattern. This strategy is faster when GADTs are rare.

   The [splitting_mode] setting determines the splitting policy.  *)
and splitting_mode =
  | Pessimistic
  (** Always split or-patterns

     We use this mode when generating exhaustivity counter-examples
     to check a refutation clause (p -> .), which typically involves
     GADTs. *)
  | Optimistic of { inside_nonsplit_or: bool; }
  (** Split or-patterns on-demand.

     In this mode we optimistically assume that splitting will not be
     necessary; we first type-check alternatives under an or-pattern
     without splitting. If we then hit a GADT constructor, we jump back
     to the or-pattern and fallback to the splitting implementation.

     We use this mode when checking exhaustivity of pattern matching;
     typical OCaml code uses few GADT constructors on average. *)


(** The exhaustivity checker produces counter-examples that contain
   synthetic/fake variant constructors and record labels, as a hackish
   way to encode a Typedtree.pattern back into a Parsetree.pattern
   without losing information. The tables below are used to read back
   the proper typed constructor/label from the synthetic parsetree-label. *)
and counter_example_synthetic_names = {
...
(** This exception is only used internally within [type_pat_aux], to jump
   back to the outermost non-split or -- in the [Optimistic] strategy. *)
...
let no_explosion = function
  ...
  | Counter_example info ->
     Counter_example { info with explode = 0 }

let get_splitting_mode = function
  | Normal -> None
  | Counter_example {splitting_mode} -> Some splitting_mode

let change_splitting_mode mode f = match mode with
  | Normal -> Normal
  | Counter_example info ->
     Counter_example { info with splitting_mode = f info.splitting_mode }
...
and type_pat_aux ~exception_allowed ~no_existentials ~mode
  ...
  let must_backtrack_on_gadt = match get_splitting_mode mode with
    | None -> false
    | Some Pessimistic -> false
    | Some (Optimistic {inside_nonsplit_or}) -> inside_nonsplit_or
  in
  match sp.ppat_desc with
    Ppat_any ->
      ...
      | Normal -> k' Tpat_any
      | Counter_example {explode; _} when explode <= 0 -> k' Tpat_any
      | Counter_example ({explode; _} as info) ->
         ...
         | (sp, constrs, labels) ->
            ...
            in
            let mode =
              Counter_example { info with
                  explode;
                  synthetic_names = { constrs; labels };
                } in
  ...
  | Ppat_construct(lid, sarg) ->
      ...
      in
      let constr =
        let synthetic_constrs = match mode with
          | Normal -> None
          | Counter_example {synthetic_names = {constrs; _}; _} ->
             Some constrs
        in
        match lid.txt, synthetic_constrs with
      ...
      | Normal ->
        k (wrap_disambiguate "This record pattern is expected to have"
             ...
             (type_label_a_list loc false !env type_label_pat opath
                lid_sp_list)
             (k' (fun x -> x)))
      | Counter_example {synthetic_names = {labels; _}; _} ->
  ...
  | Ppat_array spl ->
      ...
      map_fold_cont (fun p -> type_pat p ty_elt) spl (fun pl ->
        ...
        pat_env = !env })
  | Ppat_or(sp1, sp2) ->
      let may_split, must_split = match get_splitting_mode mode with
        | None -> false, false
        | Some Pessimistic -> true, true
        | Some (Optimistic _) -> true, false in
      ...
      if must_split then split_or sp else begin
        ...
        let env1 = ref !env in
        let inside_or =
          change_splitting_mode mode (function
              | Pessimistic -> Pessimistic
              | Optimistic _ -> Optimistic {inside_nonsplit_or = true})
        in
        let p1 =
        ...
        | None, None ->
           let inside_nonsplit_or = match get_splitting_mode mode with
             | None | Some Pessimistic -> false
             | Some (Optimistic {inside_nonsplit_or}) -> inside_nonsplit_or in
...
let check_partial ?(lev=get_current_level ()) env expected_ty loc cases =
  ...
  let splitting_mode = Optimistic {inside_nonsplit_or = false} in
...
let check_unused ?(lev=get_current_level ()) env expected_ty cases =
  Parmatch.check_unused
    (fun refute constrs labels spat ->
      match
        partial_pred ~lev ~splitting_mode:Pessimistic ~explode:5
