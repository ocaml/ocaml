...
(** In [Counter_example] mode, we are checking a counter-example candidate
    produced by Parmatch. This is a syntactic pattern that represents a set of
    values by using or-patterns (p_1 | ... | p_n) to enumerate all alternatives
    in the counter-example search.
    ...
    - it filters ill-typed branches of or-patterns (see {!splitting_mode}.
...
(**
    ...
    into concrete sub-patterns: an or-pattern of the constructors of its
    type for a variant type, or a tuple of wildcards for unary constructors,
    records, or tuples. [explosion_fuel] is the depth limit on
    wildcard explosion. Such depth limit is required to avoid
    non-termination and compilation-time blowups.

    [splitting_mode] controls the handling of or-patterns.
    In [Counter_example] mod, we only need to select one branch that leads to a
    valid sub-pattern. Checking all alternatives is expansive, we use different
    search strategies (see {!splitting_mode}) to reduce the number of explored
    alternatives.
    ...
    and label paths are lost, and are replaced by generated strings.
    Those tables map those synthetic names to their descriptions.
*)
...
and splitting_mode =
  ...
  (** Always backtrack in or-patterns
      ...
      [Backtrack_or] select a single alternative from an or-pattern by
  ...
  (** Only backtrack when needed
     ...
     constraints (because they do not contain GADT constructors).
     Those branches are such that, if they fail, all other
     branches will fail.
     If successful, it attempts to complete the subpattern, ignoring other
     branches.
