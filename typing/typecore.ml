...
let vb_exp_constraint {pvb_expr=expr; pvb_pat=pat; pvb_constraint=ct; _ } =
  ...
  | Some (Pvc_constraint { locally_abstract_univars ; univars; typ }) ->
      ...
      let constructorified = Typ.constructorify_variables univars typ in
      let expr = Exp.constraint_ ~loc expr constructorified in
      List.fold_right (Exp.newtype ~loc) (locally_abstract_univars @ univars) expr
...
let vb_pat_constraint ({pvb_pat=pat; pvb_expr = exp; _ } as vb) =
  ...
  | Some (Pvc_constraint {locally_abstract_univars; univars; typ}), _, _ ->
      let varified = Typ.varify_constructors locally_abstract_univars typ in
      let t = Typ.poly ~loc:typ.ptyp_loc (locally_abstract_univars @ univars) varified in
