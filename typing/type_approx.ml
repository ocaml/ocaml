  ...
  let approx_transl t f =
    try
      Typetexp.TyVarEnv.with_any_var_level ~level:t.mono_lvl (fun () ->
          f t.env)
    with
    | Typetexp.Error.In_context _ | Env.Error.In_context _ -> unknown t
end
...
let type_expression ~env ?mono_lvl sexp =
  let aenv = Approx_env.create ~env ?mono_lvl () in
  type_expression aenv sexp
