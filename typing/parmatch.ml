...
let rec compat p q =
  ...
  | Tpat_construct (p1,c1,ps1), Tpat_construct (p2,c2,ps2) ->
...
let rec pretty_val ppf v = match v.pat_desc with
  ...
  | Tpat_construct (path,{cstr_tag=tag},[]) ->
      let name = get_constr_name tag v.pat_type v.pat_env in
      fprintf ppf "%s" name
  | Tpat_construct (path,{cstr_tag=tag},[w]) ->
      let name = get_constr_name tag v.pat_type v.pat_env in
      fprintf ppf "@[<2>%s@ %a@]" name pretty_arg w
  | Tpat_construct (path,{cstr_tag=tag},vs) ->
      let name = get_constr_name tag v.pat_type v.pat_env in
...
| Tpat_construct (path,{cstr_tag=tag}, [_ ; _])
    when is_cons tag v ->
      fprintf ppf "(%a)" pretty_val v
| _ -> pretty_val ppf v

and pretty_cdr ppf v = match v.pat_desc with
| Tpat_construct (path,{cstr_tag=tag}, [v1 ; v2])
    when is_cons tag v ->
      fprintf ppf "%a::@,%a" pretty_car v1 pretty_cdr v2
| _ -> pretty_val ppf v
...
let simple_match p1 p2 = 
  ...
  | Tpat_construct(p1,c1, _), Tpat_construct(p2,c2, _) ->
...
| Tpat_construct(path,cstr, args) -> args
...
| (Tpat_any | Tpat_var(_)) ->
    begin match p1.pat_desc with
      Tpat_construct(path,_, args) -> omega_list args
...
let rec normalize_pat q = match q.pat_desc with
  ...
  | Tpat_construct  (path,c,args) ->
      make_pat (Tpat_construct (path,c,omega_list args)) q.pat_type q.pat_env
...
| {pat_desc = Tpat_record omegas} ->
    ...
    make_pat
      ...
      q.pat_type q.pat_env::
    rest
| {pat_desc = Tpat_construct (path,c,omegas)} ->
    let args,rest = read_args omegas r in
    make_pat
      (Tpat_construct (path,c,args)) q.pat_type q.pat_env::
    rest
...
| ({pat_desc = Tpat_construct (path,{cstr_tag=Cstr_exception _},_)},_)::_ ->
    false
| ({pat_desc = Tpat_construct(path,c,_)},_) :: _ ->
...
| ({pat_desc = Tpat_construct(path,{cstr_tag=(Cstr_constant _|Cstr_block _)},_)} as p,_) :: _ ->
...
| Some ext -> match env with
  | ({pat_desc =
       Tpat_construct(path,{cstr_tag=(Cstr_constant _|Cstr_block _)},_)} as p,_)
    :: _ ->
      let path = get_type_path p.pat_type p.pat_env in
      Path.same path ext
  | _ -> false
...
let pat_of_constr ex_pat cstr =
 {ex_pat with pat_desc = let path = match ex_pat.pat_desc with
                                    | Tpat_construct(p,c,pat) -> p
                                    | _ ->  raise Empty in
                         Tpat_construct (path,cstr,omegas cstr.cstr_arity)}
...
let build_other ext env =  match env with (* ocamlc bug ? *)
| ({pat_desc = Tpat_construct (path, ({cstr_tag=Cstr_exception _} as c), _)},_)
  ::_ ->
    make_pat
      (Tpat_construct
         (path, {c with
...
let rec le_pat p q =
  ...
  | Tpat_constant(c1), Tpat_constant(c2) -> c1 = c2
  | Tpat_construct(p1,c1,ps), Tpat_construct(p2,c2,qs) ->
...
| Tpat_construct (p1,c1,ps1), Tpat_construct (p2,c2,ps2)
      when  c1.cstr_tag = c2.cstr_tag  ->
        ...
        make_pat (Tpat_construct (p1,c1,rs)) p.pat_type p.pat_env
