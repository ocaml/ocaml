...
value caml_check_field_access(value v, value pos, value v_descr)
{
  ...
  value orig_v = v;
  if (v == (value) 0) {
    fprintf(stderr, "Access to field %" ARCH_INT64_PRINTF_FORMAT "d of NULL: %s\n",
      (ARCH_UINT64_TYPE) Long_val(pos), descr);
    abort();
  }
  if (!Is_block(v)) {
    fprintf(stderr,
      "Access to field %" ARCH_INT64_PRINTF_FORMAT "d of non-boxed value %p is illegal: %s\n",
  ...
  if (Long_val(pos) >= Wosize_val(v)) {
    fprintf(stderr,
      "Access to field %" ARCH_INT64_PRINTF_FORMAT "d of value %p of size %" ARCH_INT64_PRINTF_FORMAT "d is illegal: %s\n",
