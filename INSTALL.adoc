...
If you compile a clone of the Git repository, then run the following
command from its root directory:

        tools/autogen

Then, in any case, configure the compiler by running the following
command from the top directory:
