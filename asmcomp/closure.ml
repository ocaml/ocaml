...
let getglobal dbg id =
  let sym = Linkage_name.name (Compilenv.symbol_for_global id) in
  ...
  let app =
    ...
    | _, Never_inline | None, _ ->
      let dbg = Debuginfo.from_location loc in
        ...
        Udirect_apply(Linkage_name.Use.use fundesc.fun_label,
          app_args, dbg)
    | Some(params, body), _  ->
...
let check_constant_result lam ulam approx =
  ...
  | Value_global_field (id, i) when is_pure lam ->
      ...
      | _ ->
          let glb =
            let id = Linkage_name.name (Linkage_name.Use.name id) in
...
let intro size lam =
  reset ();
  let id = Linkage_name.Use.use (Compilenv.make_symbol None) in
