...
(* CR-soon fbour:
  ...
  Even when we know subtraction won't overflow, I don't see the point of not
  using compare directly (this applies to 1. and 2.).

  3 and 4 will require more type annotations to get optimised.
  In 4, integers are not constrained so the overflow case could a problem
  (although unlikely).
*)
