...
let rec to_clambda t env (flam : Flambda.t) : Clambda.ulambda =
  ...
  | String_switch (arg, sw, def) ->
    ...
    let sw = List.map (fun (s, e) -> s, to_clambda t env e, Debuginfo.none) sw in
    let def =
  ...
  | Try_with (body, var, handler) ->
    ...
    Utrywith (to_clambda t env body, VP.create id,
      to_clambda t env_handler handler, Debuginfo.none)
  | If_then_else (arg, ifso, ifnot) ->
    Uifthenelse (subst_var env arg,
      Debuginfo.none, to_clambda t env ifso,
      Debuginfo.none, to_clambda t env ifnot,
      Debuginfo.none)
  | While (cond, body) ->
    Uwhile (to_clambda t env cond, to_clambda t env body, Debuginfo.none)
  | For { bound_var; from_value; to_value; direction; body } ->
    ...
    Ufor (VP.create id, subst_var env from_value, subst_var env to_value,
      direction, to_clambda t env_body body, Debuginfo.none)
...
and to_clambda_switch t env cases num_keys default =
  ...
  let actions =
    Array.map (fun flam -> to_clambda t env flam, Debuginfo.none)
      (store.act_get ())
  in
  match actions with
