...
type environment =
  { vars : (Reg.t array * V.Provenance.t option) V.Map.t;
    phantom_vars : V.Set.t;
...
let dead_phantom_lets = ref V.Set.empty
...
method private lower_phantom_let var_being_defined
  ...
  | None ->
    (* The defining expression of this phantom let is never going to be
       available, perhaps because it was some expression that is not currently
       supported. *)
    None
  | Some defining_expr ->
    let defining_expr =
...
method private env_for_phantom_let env var ~defining_expr =
  (* Information about phantom lets is split at this stage:
     1. The phantom variables in scope are recorded in the environment
  ...
  | Some (provenance, defining_expr) ->
    V.Tbl.add phantom_lets var (provenance, defining_expr);
    let phantom_vars = V.Set.add var env.phantom_vars in
    { env with phantom_vars; }
...
method private extract_phantom_lets expr =
  match expr with
  | Cphantom_let (var, defining_expr, expr) ->
    let phantom_lets, expr = self#extract_phantom_lets expr in
    (var, defining_expr) :: phantom_lets, expr
  | _ -> [], expr
...
method emit_fundecl (f : Cmm.fundecl) =
  ...
  let env =
    List.fold_left (fun env (var, defining_expr) ->
        self#env_for_phantom_let env var ~defining_expr)
      env
      phantom_lets_at_top
  in
  self#emit_tail env body;
