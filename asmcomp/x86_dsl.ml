...
module I = struct
  ...
  let movdqa x y = emit (MOVDQA (x, y))
  let movdqu x y = emit (MOVDQU (x, y))
  ...
  let vmovapd x y = emit (VMOVAPD (x, y))
  let vmovdqa x y = emit (VMOVDQA (x, y))
  let vmovdqu x y = emit (VMOVDQU (x, y))
  let vmovupd x y = emit (VMOVUPD (x, y))
