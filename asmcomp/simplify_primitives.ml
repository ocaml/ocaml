...
let primitive (p : Lambda.primitive) (args, approxs) expr dbg
  ...
  | _ ->
    ...
    | [Value_constptr x] ->
      ...
      | Pctconst c ->
        begin match c with
        | Big_endian -> S.const_bool_expr expr Arch.big_endian eid
        | Word_size -> S.const_int_expr expr (8*Arch.size_int) eid
        | Int_size -> S.const_int_expr expr (8*Arch.size_int - 1) eid
        | Max_wosize ->
          (* CR mshinwell: this function should maybe not live here. *)
          S.const_int_expr expr ((1 lsl ((8*Arch.size_int) - 10)) - 1) eid
