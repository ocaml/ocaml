...
let pseudoregs_for_operation op arg res =
  ...
  | Iextcall { func; alloc = false; }
      when S.equal func __aeabi_idivmod ->
      (arg, [|r1|])
...
method! is_simple_expr = function
  ...
  | Cop(Cextcall(func, _, _, _), args, _)
      when !fpu >= VFPv2 && S.equal func sqrt ->
      List.for_all self#is_simple_expr args
  (* inlined byte-swap ops are simple if their arguments are *)
  | Cop(Cextcall(func, _, _, _), args, _)
    when !arch >= ARMv6T2 && S.equal func caml_bswap16_direct ->
      List.for_all self#is_simple_expr args
  | Cop(Cextcall(func, _, _, _), args, _)
    when !arch >= ARMv6 && S.equal func (caml_direct_bswap Int32) ->
      List.for_all self#is_simple_expr args
...
method private select_operation_vfpv3 op args dbg =
  ...
  | (Cextcall(func, _, false, _), args) when S.equal func sqrt ->
