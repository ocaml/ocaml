...
type t = {
  name : Asm_label.t;
  entries : Range_list_entry.t list;
}

(* Same note as in location_list.ml. *)
let sort entries =
  List.sort Range_list_entry.compare_ascending_vma entries
...
let size t =
  ...
  let body_size =
    List.fold_left (fun size entry -> size + (Range_list_entry.size entry))
      (Dwarf_int.zero ())
      t.entries
  in
...
let compare_increasing_vma t1 t2 =
  ...
  | t1_entry::_, t2_entry::_ ->
    Range_list_entry.compare_ascending_vma t1_entry t2_entry
...
let emit t =
  ...
  List.iter (fun entry -> Range_list_entry.emit entry) t.entries;
