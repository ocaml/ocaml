...
let instr register_name print_addressing print_specific_operation ppf i =
  ...
  | Lcondbranch(tst, lbl) ->
      fprintf ppf "if %a goto %a" (test register_name tst) i.arg label lbl
  | Lcondbranch3(lbl0, lbl1, lbl2) ->
      fprintf ppf "switch3 %a" (reg register_name) i.arg.(0);
      let case n = function
      | None -> ()
      | Some lbl ->
...
let rec all_instr register_name print_addressing print_specific_operation ppf i =
  let all_instr = all_instr register_name print_addressing print_specific_operation in
  match i.desc with
  | Lend -> ()
  | _ -> fprintf ppf "%a@,%a" (instr register_name print_addressing print_specific_operation) i all_instr i.next
...
let fundecl register_name print_addressing print_specific_operation ppf f =
  ...
  fprintf ppf "@[<v 2>%s:%s@,%a@]" f.fun_name dbg (all_instr register_name print_addressing print_specific_operation) f.fun_body
