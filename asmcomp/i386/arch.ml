...
type specific_operation =
  ...
  | Istore_symbol of Linkage_name.t * addressing_mode * bool
                                        (* Store a symbol *)
  | Ioffset_loc of int * addressing_mode (* Add a constant to a location *)
  | Ipush                               (* Push regs on stack *)
  | Ipush_int of nativeint              (* Push an integer constant *)
  | Ipush_symbol of Linkage_name.t      (* Push a symbol *)
  ...
  | Ifloatspecial of Linkage_name.t
...
let print_specific_operation printreg op ppf arg =
  ...
  | Istore_symbol(lbl, addr, is_assign) ->
      fprintf ppf "[%a] := \"%a\" %s"
         (print_addressing printreg addr) arg Linkage_name.print lbl
         (if is_assign then "(assign)" else "(init)")
  | Ioffset_loc(n, addr) ->
  ...
  | Ipush_symbol s ->
      fprintf ppf "push \"%a\"" Linkage_name.print s
  | Ipush_load addr ->
  ...
  | Ifloatspecial name ->
      fprintf ppf "%a " Linkage_name.print name;
      for i = 0 to Array.length arg - 1 do
        if i > 0 then fprintf ppf ", ";
        printreg ppf arg.(i)
      done
