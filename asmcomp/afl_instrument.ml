...
let afl_area_ptr = Cconst_symbol (LU.caml_afl_area_ptr)
let afl_prev_loc = Cconst_symbol (LU.caml_afl_prev_loc)
...
let instrument_initialiser c =
  ...
  with_afl_logging
    (Csequence
       (Cop (Cextcall (LU.caml_setup_afl, typ_int, false, None),
             [Cconst_int 0],
             Debuginfo.none),
        c))
