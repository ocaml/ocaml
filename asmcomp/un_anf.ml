...
let make_ident_info (clam : Clambda.ulambda) : ident_info =
  ...
  let rec loop : Clambda.ulambda -> unit = function
    ...
    | Uoffset (expr, offset) ->
      loop expr;
      ignore_int offset
    | Ulet (_ident, def, body, _block_kind) ->
      loop def;
      loop body
    | Uletrec (defs, body) ->
...
let let_bound_vars_that_can_be_moved ident_info (clam : Clambda.ulambda) =
  ...
  let rec loop : Clambda.ulambda -> unit = function
    ...
    | Uoffset (expr, offset) ->
      ...
      ignore_int offset
    | Ulet (ident, def, body, _block_kind) ->
...
let rec substitute_let_moveable is_let_moveable env (clam : Clambda.ulambda)
  ...
  | Ulet (id, def, body, block_kind) ->
    ...
    if Ident.Set.mem id is_let_moveable then
      ...
      Ulet (id, def, substitute_let_moveable is_let_moveable env body, block_kind)
  | Uletrec (defs, body) ->
    let defs =
      List.map (fun (id, def) ->
...
let rec un_anf_and_moveable ident_info env (clam : Clambda.ulambda)
  ...
  | Ulet (id, def, Uvar id', _block_kind) when Ident.same id id' ->
  ...
  | Ulet (id, def, body, block_kind) ->
    ...
    | Fixed, _, _ ->
      ...
      Ulet (id, def, body, block_kind), both_moveable def_moveable body_moveable
    end
  | Uletrec (defs, body) ->
    let defs =
