...
(*   | Block (tag, fields) -> *)
(*     let approxs = List.map (describe_constant env) fields in *)
...
(* let constant_sets_of_closures_descr env program = *)
(*   let add_set_of_closure acc (symbol, (c:Flambda.constant_defining_value)) = *)
(*     match c with *)
(*     | Set_of_closures set_of_closures -> *)
(*       let env = { env with var = Variable.Map.empty } in *)
(*       Symbol.Map.add symbol *)
(*         (describe_set_of_closures env set_of_closures) *)
(*         acc *)
...
(* let record_project_closures env (set_of_closures:Export_info.value_set_of_closures) = *)
(*   (\* TODO: drop ! *\) *)
(*   (\* Closure_id.Map.iter (fun closure_id _ -> *\) *)
(*   (\*     let symbol = Compilenv.closure_symbol closure_id in *\) *)
(*   (\*     let export_id = *\) *)
(*   (\*       new_descr env (Value_closure { closure_id; set_of_closures }) *\) *)
(*   (\*     in *\) *)
(*   (\*     new_symbol env symbol export_id) *\) *)
...
*)
  (* let constant_sets_of_closures_descr = *)
  (*   Flambda_utils.all_lifted_constant_sets_of_closures program *)
  (* in *)
  (* constant_sets_of_closures_descr env program in *)
  ...
  (*     new_symbol symbol (new_descr env (Export_info.Value_set_of_closures descr)) *)
  (*   ) *)
...
(*         ) *)
(*     in *)
(*     new_descr env (Value_block (Tag.zero,fields)) *)
(*   in *)
