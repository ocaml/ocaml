                                ...
                                unsigned char * opt_digest)
{
  ...
  caml_lf_skiplist_insert(&code_fragments_by_pc,
                       (uintnat) start, (uintnat) cf);
  caml_lf_skiplist_insert(&code_fragments_by_num,
...
void caml_remove_code_fragment(struct code_fragment * cf)
{
  ...
  if( caml_lf_skiplist_remove(&code_fragments_by_num, cf->fragnum) ) {
    cf_cell = (struct code_fragment_garbage*)caml_stat_alloc(sizeof(struct code_fragment_garbage));

    cf_cell->cf = cf;

    do {
      cf_cell->next = atomic_load_explicit(&garbage_head, memory_order_acquire);
    } while( !atomic_compare_exchange_strong(&garbage_head, &cf_cell->next, cf_cell) );
  }
}
...
struct code_fragment * caml_find_code_fragment_by_pc(char *pc)
{
  ...
  if (caml_lf_skiplist_find_below(&code_fragments_by_pc,
                               (uintnat) pc, &key, &data)) {
