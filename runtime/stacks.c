...
CAMLexport caml_value * caml_stack_low;
CAMLexport caml_value * caml_stack_high;
CAMLexport caml_value * caml_stack_threshold;
CAMLexport caml_value * caml_extern_sp;
CAMLexport caml_value * caml_trapsp;
CAMLexport caml_value * caml_trap_barrier;
caml_value caml_global_data = 0;

uintnat caml_max_stack_size;            /* also used in gc_ctrl.c */

void caml_init_stack (uintnat initial_max_size)
{
  caml_stack_low = (caml_value *) caml_stat_alloc(Stack_size);
  caml_stack_high = caml_stack_low + Stack_size / sizeof (caml_value);
  caml_stack_threshold = caml_stack_low + Stack_threshold / sizeof (caml_value);
  ...
  caml_gc_message (0x08, "Initial stack limit: %"
                   ...
                   caml_max_stack_size / 1024 * sizeof (caml_value));
}

void caml_realloc_stack(asize_t required_space)
{
  asize_t size;
  caml_value * new_low, * new_high, * new_sp;
  caml_value * p;
  ...
  caml_gc_message (0x08, "Growing stack to %"
                         ARCH_INTNAT_PRINTF_FORMAT "uk bytes\n",
                   (uintnat) size * sizeof(caml_value) / 1024);
  new_low = (caml_value *) caml_stat_alloc(size * sizeof(caml_value));
  new_high = new_low + size;

#define shift(ptr) \
    ((char *) new_high - ((char *) caml_stack_high - (char *) (ptr)))

  new_sp = (caml_value *) shift(caml_extern_sp);
  memmove((char *) new_sp,
          (char *) caml_extern_sp,
          (caml_stack_high - caml_extern_sp) * sizeof(caml_value));
  caml_stat_free(caml_stack_low);
  caml_trapsp = (caml_value *) shift(caml_trapsp);
  caml_trap_barrier = (caml_value *) shift(caml_trap_barrier);
  for (p = caml_trapsp; p < new_high; p = Trap_link(p))
    Trap_link(p) = (caml_value *) shift(Trap_link(p));
...
CAMLprim caml_value caml_ensure_stack_capacity(caml_value required_space)
{
