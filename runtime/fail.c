...
CAMLexport value caml_constant_exn(value tag)
{
  return Make_exception_result(tag);
}

CAMLexport value caml_with_arg_exn(value tag, value arg)
{
  CAMLparam2 (tag, arg);
  CAMLlocal1 (bucket);
...
CAMLexport value caml_with_args_exn(value tag, int nargs, value args[])
{
  CAMLparam1 (tag);
...
CAMLexport value caml_with_string_exn(value tag, char const *msg)
{
  CAMLparam1(tag);
  value v_msg = caml_copy_string(msg);
  CAMLreturn(caml_with_arg_exn(tag, v_msg));
}


static CAMLnoret void raise_encoded(value exn)
{
...
{
  raise_encoded(caml_sys_blocked_io_exn());
}
