...
{
  // musl's memcmp is not optimized and works on bytes
  // so use our own word-sized implementation.
  // To check if musl updated their implementation:
  // http://git.musl-libc.org/cgit/musl/tree/src/string/memcmp.c
      ...
      sz2 = Bosize_val(s2);
      if (sz1 != sz2) return Val_false;
      // __memcmpeq has the same semantics as memcmp
      // when only its return value truthfullness is
      // considered, but is more efficient.
      #ifdef HAS_MEMCMPEQ
        res = __memcmpeq(Op_val(s1), Op_val(s2), sz1);
      #else
        res = memcmp(Op_val(s1), Op_val(s2), sz1);
      #endif
      if (res == 0) return Val_true;
      return Val_false;
  #endif
}
...
{
  return Val_not(caml_bytes_equal(s1, s2));
}
