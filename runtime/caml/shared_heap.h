...
extern struct caml_heap_state *caml_init_shared_heap(void);
extern void caml_teardown_shared_heap(struct caml_heap_state *heap);

extern value *caml_shared_try_alloc(struct caml_heap_state *,
                                    mlsize_t, tag_t, reserved_t, int);

/* Copy the domain-local heap stats into a heap stats sample. */
extern void caml_collect_heap_stats_sample(
    struct caml_heap_state *local,
    struct heap_stats *sample);

/* Add the global orphaned heap stats into an accumulator. */
extern void caml_accum_orphan_heap_stats(struct heap_stats *acc);

extern uintnat caml_heap_size(struct caml_heap_state *);
extern uintnat caml_top_heap_words(struct caml_heap_state *);
extern uintnat caml_heap_blocks(struct caml_heap_state *);

extern struct pool *caml_pool_of_shared_block(value v);

extern void caml_shared_unpin(value v);
...
extern void caml_redarken_pool(struct pool *, scanning_action, void *);

extern intnat caml_sweep(struct caml_heap_state *, intnat);

/* must be called during STW */
extern void caml_cycle_heap_stw(void);

/* must be called on each domain
   (after caml_cycle_heap_stw) */
extern void caml_cycle_heap(struct caml_heap_state *);

/* Heap invariant verification (for debugging) */

/* caml_verify_heap must only be called while all domains are paused */
extern void caml_verify_heap(caml_domain_state *domain);
