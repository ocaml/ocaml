...
class printer  ()= object(self:'self)
  ...
  method structure_item f x = begin
    ...
    | Pstr_extension te -> self#type_extension f te
    ...
    | Pstr_class l ->
        let class_declaration f  (* for the second will be changed to and FIXME*)
            ({pci_params=ls;
              ...
              pci_expr={pcl_desc;_};
              pci_variance;_ } as x) =
          let ls = List.combine pci_variance ls in
  ...
  method type_param f  = function
    | (a,ct) -> pp f "%s%a" (type_variance a) self#core_type ct
