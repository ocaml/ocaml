...
let unsigned_div n d =
  ...
  let d = if d < 0 then d + 65536 else d in
  of_int (n / d)
let rem = ( mod )
...
let unsigned_to_int n = Some (if n >= 0 then n else n + 65536)

let of_string s =
  match Stdlib.int_of_string_opt s with
  | Some n when n >= min_int && n <= max_int -> n
  | Some n when n > max_int && n <= 65535
      && s.[0] = '0' && (s.[1] = 'u' || s.[1] = 'U') -> n - 65536
  | _ -> failwith "Int16.of_string"
