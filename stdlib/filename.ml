...
module Win32_deps : SYSDEPS = struct
  let null = "NUL"
  let current_dir_name = "."
  let parent_dir_name = ".."
  let dir_sep = "\\"
...
module Cygwin_deps : SYSDEPS = struct
  let null = "/dev/null"
  let current_dir_name = "."
  let parent_dir_name = ".."
  let dir_sep = "/"
  let is_dir_sep = Win32_deps.is_dir_sep
  let is_relative = Win32_deps.is_relative
  let is_implicit = Win32_deps.is_implicit
  let check_suffix = Win32_deps.check_suffix
  let chop_suffix_opt = Win32_deps.chop_suffix_opt
  let temp_dir_name = Unix_deps.temp_dir_name
  let quote = Unix_deps.quote
  let quote_command = Unix_deps.quote_command
  let basename = generic_basename is_dir_sep current_dir_name
  let dirname = generic_dirname is_dir_sep current_dir_name
end

module Unix = Make (Unix_deps)

module Win32 = Make (Win32_deps)
