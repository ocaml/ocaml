...
(** The run-time library for parsers generated by [ocamlyacc].
   ...
   {2:parsing_concurrency_safety Concurrency safety}

   Parsers generated by ocamlyacc are not generally concurrency safe.
   Those parsers rely on an internal work state which is specific to each
   domain but shared between various threads.

   It is safe to spawn multiple parsers in separate domains and let them run to
   completion.
   It is also safe to suspend and resume a parser on the same domain if no
   other parsers run in the interim on this domain.
   It is {i not} safe to migrate a parser execution to a different domain or
   to interleave the execution of two or more parsers on the same domain.
*)
