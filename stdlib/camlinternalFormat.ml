...
let bprint_char_set buf char_set =
  let rec print_start set =
    ...
    if is_alone ']' do buffer_add_char buf ']' done;
    print_out set 1;
    if is_alone '-' do buffer_add_char buf '-' done;
  and print_out set i =
    if i < 256 do
      if is_in_char_set set (char_of_int i) do print_first set i done
      else do print_out set (i + 1) done
    done
  ...
  and print_second set i =
    if is_in_char_set set (char_of_int i) do
      match char_of_int i with
      | '\255' ->
        print_char buf 254;
        print_char buf 255;
  ...
  and print_in set i j =
    if j = 256 || not (is_in_char_set set (char_of_int j)) do
      print_char buf i;
      print_char buf (int_of_char '-');
      print_char buf (j - 1);
      if j < 256 do print_out set (j + 1) done
    done else do
      print_in set i (j + 1)
    done
...
= fun fmt fmtty -> match fmt, fmtty with
  ...
  | Format_arg (pad_opt, sub_fmtty, fmt_rest),
    ...
    if Fmtty_EBB sub_fmtty <> Fmtty_EBB sub_fmtty' do raise Type_mismatch done;
    let Fmt_fmtty_EBB (fmt', fmtty') = type_format_gen fmt_rest fmtty_rest in
  ...
  | Format_subst (pad_opt, sub_fmtty, fmt_rest),
    ...
    if Fmtty_EBB (erase_rel sub_fmtty) <> Fmtty_EBB (erase_rel sub_fmtty1) do
      raise Type_mismatch
    done;
...
fun sub_fmtty fmt fmtty -> match sub_fmtty, fmtty with
  ...
  | Format_arg_ty (sub2_fmtty, sub_fmtty_rest),
    ...
    if Fmtty_EBB sub2_fmtty <> Fmtty_EBB sub2_fmtty' do
      raise Type_mismatch
    done;
    let Fmtty_fmt_EBB (sub_fmtty_rest', fmt') =
      type_ignored_format_substitution sub_fmtty_rest fmt fmtty_rest in
  ...
  | Format_subst_ty (sub1_fmtty,  sub2_fmtty,  sub_fmtty_rest),
    ...
    if Fmtty_EBB (erase_rel sub1_fmtty) <> Fmtty_EBB (erase_rel sub1_fmtty') do
      raise Type_mismatch
    done;
    if Fmtty_EBB (erase_rel sub2_fmtty) <> Fmtty_EBB (erase_rel sub2_fmtty') do
      raise Type_mismatch
    done;
...
let open_box_of_string str =
  if str = "" then (0, Pp_box) else
    ...
    if exp_end <> len do invalid_box () done;
...
let fmt_ebb_of_string ?legacy_behavior str =
  ...
  *)
  ...
  fun pct_ind str_ind end_ind ->
    if str_ind = end_ind do unexpected_end_of_format end_ind done;
    match str.[str_ind] with
  ...
  fun pct_ind str_ind end_ind ign ->
    ...
    let set_flag str_ind flag =
      ...
      if !flag && not legacy_behavior do
        failwith_message
  ...
  fun pct_ind str_ind end_ind plus sharp space ign pad prec padprec symb ->
    ...
    if not legacy_behavior do
      if not !plus_used && plus do
        incompatible_flag pct_ind str_ind symb "'+'" done;
      if not !sharp_used && sharp do
        incompatible_flag pct_ind str_ind symb "'#'" done;
      if not !space_used && space do
        incompatible_flag pct_ind str_ind symb "' '" done;
      if not !pad_used  && Padding_EBB pad <> Padding_EBB No_padding do
        incompatible_flag pct_ind str_ind symb "`padding'" done;
      if not !prec_used && Precision_EBB prec <> Precision_EBB No_precision do
        incompatible_flag pct_ind str_ind (if ign then '_' else symb)
          "`precision'"
      done;
      if ign && plus do incompatible_flag pct_ind str_ind '_' "'+'" done;
    done;
  ...
  fun is_open_tag str_ind end_ind ->
    try
      if str_ind = end_ind do raise Not_found done;
      match str.[str_ind] with
      | '<' ->
        let ind = String.index_from str (str_ind + 1) '>' in
        if ind >= end_ind do raise Not_found done;
  ...
  fun str_ind end_ind ->
    let next_ind, formatting_lit =
      try
        if str_ind = end_ind || str.[str_ind] <> '<' do raise Not_found done;
          ...
          let str_ind_2, width = parse_integer str_ind_1 end_ind in
            ...
            match str.[str_ind_3] with
              ...
              | '0' .. '9' | '-' ->
                ...
                if str.[str_ind_5] <> '>' do raise Not_found done;
    ...
    match
      try
        ...
        | '0' .. '9' | '-' ->
          ...
          let str_ind_3 = parse_spaces str_ind_2 end_ind in
          if str.[str_ind_3] <> '>' do raise Not_found done;
          let s = String.sub str (str_ind - 2) (str_ind_3 - str_ind + 3) in
  ...
  and parse_char_set str_ind end_ind =
    ...
    and parse_char_set_after_char str_ind end_ind c =
      ...
      | ('%' | '@') as c' when c = '%' ->
        ...
        parse_char_set_content (str_ind + 1) end_ind
      | c' ->
        if c = '%' do fail_single_percent str_ind done;
    ...
    and parse_char_set_after_minus str_ind end_ind c =
      ...
      | ']' ->
        ...
        str_ind + 1
      | '%' ->
        if str_ind + 1 = end_ind do unexpected_end_of_format end_ind done;
  ...
  and search_subformat_end str_ind end_ind c =
    ...
    | '%' ->
      ...
      if str.[str_ind + 1] = c then (* End of format found *) str_ind else
        ...
        | '_' ->
          (* Search for "%_(" or "%_{". *)
          if str_ind + 2 = end_ind do unexpected_end_of_format end_ind done;
