...
let filter p arg =
  ...
  if size_arg = 0 then [| |] else
    ...
    if !size_res = 0 then [| |]
    else if !size_res = size_arg then unsafe_sub arg 0 size_arg
    else
      let result = make !size_res (unsafe_get arg 0) in
      let i_res = ref 0 in
      for i_bv = 0 to (size_arg lsr 3) - 1 do
        let c = Char.code (Bytes.unsafe_get bv i_bv) in
      ...
      if size_arg land 7 > 0 then begin
        let c = Char.code (Bytes.unsafe_get bv (size_arg lsr 3)) in
