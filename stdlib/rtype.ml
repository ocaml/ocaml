...
let string_of_digest_type = 
  string_of_val_type (fun d -> "[" ^ string_of_digest d ^ "]" );;

let string_of_run_type = 
  string_of_val_type (fun (p,dt) -> 
    string_of_run_ident p ^ "(= " ^ string_of_digest_type dt ^ ")" );;

let rec digest_type_of_run_type = function
  | Rtyp_var x -> Rtyp_var x
  | Rtyp_arrow (l,t1,t2) -> 
      Rtyp_arrow (l, digest_type_of_run_type t1,
		     digest_type_of_run_type t2)
  | Rtyp_tuple tls -> Rtyp_tuple (List.map digest_type_of_run_type tls)
  | Rtyp_constr ((_,dt),_) -> dt
;;
...
let is_instance comppath is_same g t =
...
(*  | _, Rtyp_var i when i < 0 -> [] (* _ i.e. "any" type pattern *) *)
    | Rtyp_var i, _ ->
  	begin 
  	  try
  	    if is_same (List.assoc i subst) t then subst else raise Exit
  	  with
  	    Not_found -> (i, t) :: subst
  	end
...
let is_instance_digest_type = is_instance (=) (=);;

let is_instance_run_type = 
  let path_pairs = ref [] in
  let is_same t1 t2 = 
    let rec aux t1 t2 =
      match t1, t2 with
      | Rtyp_var i, Rtyp_var j when i = j -> ()
      | Rtyp_arrow (l1,f1,t1), Rtyp_arrow (l2,f2,t2) ->
	  if l1 <> l2 then raise Exit;
	  aux f1 f2; 
	  aux t1 t2
      | Rtyp_tuple tl1, Rtyp_tuple tl2 
	when List.length tl1 = List.length tl2 ->
	  List.iter2 aux  tl1 tl2
      | Rtyp_constr ((rp1,_), tl1), Rtyp_constr ((rp2,_), tl2) 
	when rp1 = rp2 ->
	  List.iter2 aux tl1 tl2
      | _ -> raise Exit
    in
    try aux t1 t2; true with Exit -> false
  in
  is_instance (fun (rp1,_) (rp2,_) -> rp1 = rp2) is_same
;;
...
let dynamic_comp =
  fun [|ty|] v ->
    (* The run time type is given as an array [|ty|], so that
       we will be able to make the construct [dyn e] be a normal function
...
let coerce_comp = fun (m,ls,le) [|ty1|] ((v, ty2) as d) ->
  let dty1 = digest_type_of_run_type ty1 in
  let dty2 = digest_type_of_run_type ty2 in
  if is_instance_digest_type dty2 dty1 && is_instance_run_type ty2 ty1 
  then (v : 'a) 
...
let import_comp = fun (m,ls,le) [|ty1|] ((v, ty2) as d) ->
  let dty1 = digest_type_of_run_type ty1 in
  let dty2 = digest_type_of_run_type ty2 in
  if is_instance_digest_type dty2 dty1 then (v : 'a) 
  else fail ty2 ty1 m ls le
