...
let format6 = (Obj.magic Builtintypes.format6 : type_declaration)
...
(*
(* dummy code for porting G'Caml to the latest O'Caml *)
let int = (Obj.magic dummy : type_declaration)
let char = (Obj.magic dummy : type_declaration)
let string = (Obj.magic dummy : type_declaration)
let float = (Obj.magic dummy : type_declaration)
let bool = (Obj.magic dummy : type_declaration)
let unit = (Obj.magic dummy : type_declaration)
let exn = (Obj.magic dummy : type_declaration)
let array = (Obj.magic dummy : type_declaration)
let list = (Obj.magic dummy : type_declaration)
let format6 = (Obj.magic dummy : type_declaration)
let option = (Obj.magic dummy : type_declaration)
let nativeint = (Obj.magic dummy : type_declaration)
let int32 = (Obj.magic dummy : type_declaration)
let int64 = (Obj.magic dummy : type_declaration)
let lazy_t = (Obj.magic dummy : type_declaration)
*)

let builtin_types = [int; char; string; float; bool; unit; exn; array; list; format6; option; nativeint; int32; int64; lazy_t]
...
let print_related_type_declarations ppf decl =
  raw_print_type_declarations (fun ppf (p,_) -> print_path ppf p) ppf
    (decl :: (List.map (fun v -> (unbox (snd v))) decl.type_defined_with))
