...
(* exposed as 'external' so the primitive reaches the call site, where the
   type is usually known. On an immediate the compiler can then skip the
   write barrier and emit the bare instruction. *)
